...
\begin{abstract}
Interstellar comets 2I/Borisov and 3I/ATLAS, typically observed at SNR~$\sim$1--5, lie far below the regime where traditional single-modality cometary analysis methods have been validated. We present \textsc{Cometh}, a physics-constrained multimodal fusion framework integrating: a multi-scale CNN with squeeze-and-excitation attention for faint-feature detection (2.8$\times$ SNR gain over traditional image-processing baselines at SNR~$\sim$2); a physics-informed neural network (PINN) with hard mass, momentum, and angular momentum conservation constraints for parameter inversion ($\sim$8\% error on dust production rate on synthetic benchmarks; $\sim$12\% mean absolute deviation against published 2I/Borisov values); and a cross-modal attention module with out-of-distribution detection preventing silent extrapolation failures. Domain adaptation via physically motivated meta-learning reduces synthetic-to-synthetic domain shift (MMD 0.47$\to$0.12); the synthetic-to-real gap remains unclosed pending multi-object samples. A same-data direct comparison on 231 HST/WFC3 and VLT/FORS2 images of 2I/Borisov yields 201 Af$\rho$(1000\,km) measurements across 11 epochs; HST and FORS2 agree within $\sim$3\% (ratio 1.03), validating the traditional-method baseline for solar-system-like compositions. A systematic stress-test suite maps failure boundaries; the spherical-grain assumption ($\sim$8\% bias on $Q_d$) is the primary systematic limitation. A Disaster Preview shows that traditional pipelines would misinterpret extreme-composition comets by factors up to 50$\times$, and an illustrative pilot study finds human experts miss $>2/3$ of faint cometary features at SNR~$<2$. All ``3I/ATLAS'' results are synthetic case studies. Code is available at \url{https://anonymous.4open.science/r/cometh-review-XXXXXX}; trained weights are archived at Zenodo (DOI embargoed until acceptance).
\end{abstract}
...
\subsubsection{Cross-Target Context: 3I/ATLAS Parameter Space Coverage}\label{sec:3i_atlas}

\textbf{We have not run the \textsc{Cometh} pipeline on 3I/ATLAS data.} Calibrated multi-modal archives of 3I/ATLAS are not yet available, and the frozen model weights required for inference remain under Zenodo embargo. This section provides context only: we check whether the published compositional parameters of the second known interstellar comet fall within \textsc{Cometh}'s pre-defined training envelope.

Published JWST/NIRSpec spectroscopy of 3I/ATLAS reports CO$_2$/H$_2$O = $7.6 \pm 0.3$ \citep{cordiner25}. This value lies within \textsc{Cometh}'s PINN training range (CO/H$_2$O $\in [0, 10]$; Stress Test~5 uses CO/H$_2$O = 5.0) by design---the range was constructed to span the plausible interstellar comet parameter space based on physical priors, not learned from data. The OOD detector flags compositions with CO/H$_2$O $> 2$ (\S\ref{sec:ood}), so 3I/ATLAS's extreme CO$_2$ enrichment would trigger the CI-inflation safeguard, correctly alerting users that the estimate falls outside the validated (as opposed to the trained) envelope. Additional published diagnostics---water-ice grains \citep{yang25}, $Q$(H$_2$O) $\sim 3\times10^{29}$~s$^{-1}$ \citep{combi26}, SPHEREx CO$_2$/H$_2$O mapping \citep{lisse26}, trace species detections \citep{biver26,roth26}, and declining Ni/Fe \citep{hutsemekers26}---confirm that 3I/ATLAS is spectroscopically distinct from 2I/Borisov (CO$_2$-rich vs.\ CO-rich), reinforcing that the known interstellar comet population spans a wide compositional range.

\textbf{Limitation:} This cross-check confirms only that the training range was designed broadly enough to cover the two known interstellar comets---a design choice, not an empirical validation. Running \textsc{Cometh} inference on calibrated 3I/ATLAS data would constitute a genuine second-object consistency check ($N=2$); we defer this to future work when calibrated archives and model weights are available.
