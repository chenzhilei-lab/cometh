...
\begin{abstract}
Interstellar comets 2I/Borisov and 3I/ATLAS, typically observed at SNR~$\sim$1--5, lie far below the regime where traditional single-modality cometary analysis methods have been validated. We present \textsc{Cometh}, a physics-constrained multimodal fusion framework integrating: a multi-scale CNN with squeeze-and-excitation attention for faint-feature detection (2.8$\times$ SNR gain); a physics-informed neural network (PINN) with hard mass, momentum, and angular momentum conservation constraints for parameter inversion ($\sim$8\% error on $Q_d$); and cross-modal attention with out-of-distribution detection preventing silent extrapolation failures. Domain adaptation via physically motivated meta-learning bridges the synthetic-to-real gap. A same-data comparison on 231 HST/WFC3 and VLT/FORS2 images of 2I/Borisov yields 201 Af$\rho$(1000\,km) measurements; HST and FORS2 agree within $\sim$3\% (ratio 1.03), validating the traditional-method baseline. A stress-test suite maps failure boundaries; the spherical-grain assumption ($\sim$8\% bias on $Q_d$) is the primary systematic limitation. A Disaster Preview shows that traditional pipelines would misinterpret extreme-composition comets by factors up to 50$\times$, and an illustrative pilot study finds human experts miss $>2/3$ of faint cometary features at SNR~$<2$. All ``3I/ATLAS'' results are synthetic case studies. Code is available at \url{https://anonymous.4open.science/r/cometh-review-XXXXXX}; trained weights are archived at Zenodo (DOI embargoed until acceptance).
\end{abstract}
...
class ComethFramework:
    ...
    def process_observation(
        ...
        # Returns Q_dust, alpha, a_max,
        # CO_H2O, C2_CN, detection_map,
        # gradcam_map, ood_flag,
        # ood_confidence, reliability.
        # Raises ValueError (dims out of
        # range) or OODWarning.
        ...
\end{verbatim}
}
