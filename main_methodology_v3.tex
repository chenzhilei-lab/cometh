...
\title[A Deep Learning Framework for Faint Comet Characterization]{A Benchmarking Study of Deep Learning Methods for Faint Comet Characterization: Where Traditional Methods Fail and Physics-Informed Neural Networks Excel}
...
\begin{abstract}
Interstellar comets 2I/Borisov and 3I/ATLAS are typically observed at SNR~$\sim$1--5, far below the regime where traditional cometary analysis methods---$Af\rho$ aperture photometry, Haser-model fitting, and manual morphological inspection---have been validated. We present a controlled human vs.\ machine experiment showing that expert astronomers achieve only a 31\% true-positive rate for feature detection at SNR~$<2$ (inter-expert agreement $\kappa = 0.41$), demonstrating that traditional manual analysis is unreliable on data this faint. We introduce \textsc{Cometh}, a simulation-based deep learning framework that combines multi-scale CNNs for faint-feature detection with physics-informed neural networks whose architectures enforce mass, momentum, and angular momentum conservation. Trained entirely on synthetic data and frozen before application, \textsc{Cometh} achieves a 2.8$\times$ SNR gain over standard image-processing methods on synthetic benchmarks. Applied to archival 2I/Borisov data, the framework recovers dust production rates within $\sim$12\% of published $Af\rho$-derived values and correctly identifies the object's extreme CO enrichment. A systematic stress-test suite maps the failure boundaries of both traditional methods and the framework, identifying an absolute detection floor at SNR~$\sim 0.75$ and a reliable inversion threshold at SNR~$\sim 3$. All performance metrics are evaluated against known synthetic ground truth; the 2I/Borisov comparison ($N=1$) is a feasibility demonstration, not a statistical validation. The primary systematic limitation---the spherical-grain approximation---introduces an $\sim$8\% bias on dust production rates, which we quantify. Code, trained weights, synthetic datasets, and an observing-strategy optimization tool are publicly available.
\end{abstract}
...
Interstellar objects passing through the Solar System offer a direct probe of extrasolar planetary system material. Extracting physical information from their observations, however, pushes data analysis methods to---and often beyond---their validated limits. Stated simply: interstellar comets are usually discovered when they are far from the Sun and moving against crowded star fields, which makes their images noisy and hard to analyze with standard tools. Three factors conspire to create this difficulty:
...
This paper is a \textit{benchmarking and methodology study}. Its first contribution is a systematic stress test suite (\S\ref{sec:stress}) that probes the failure boundaries of cometary analysis methods under deliberately difficult conditions---extreme background star fields, high optical depth inner comae, ultra-low SNR ($\leq 1$), missing data modalities, and target compositions far outside the solar system training distribution. Second, we present a multi-modal deep learning framework (\S\ref{sec:methods}) built around six interconnected innovations: multi-scale CNNs with SE attention, integrated Grad-CAM explainability, and an attention consistency loss that trains the network to focus on physically meaningful pixels; PINNs with hard physical constraints---mass, momentum, and angular momentum conservation, plus a dust-gas drag ODE---embedded directly in the architecture; physically motivated meta-learning with adversarial domain adaptation; a self-diagnosing OOD detector (\S\ref{sec:ood}) that prevents silent extrapolation failures by flagging inputs outside the training distribution; an adversarial observing strategy optimizer (\S\ref{sec:obsopt}) with a reinforcement learning extension for dynamic cadence adjustment; and non-spherical grain parameters (porosity, fractal dimension) handled by a layered PINN architecture for optically thick coma conditions.
...
\bottomrule
\end{tabular}
\tablecomments{Each stress test evaluated on $N_{\rm test}
\end{table*}
...
\bottomrule
\end{tabular}
\tablecomments{All learning rates were determined by LR range test \citep{smith17}
\end{table*}
...
\bottomrule
\end{tabular}
\tablecomments{All traditional-method values are from published literature, not from re-analysis of the same raw data. Framework results are from frozen inference with no fine-tuning on 2I/Borisov. The $Q_d$ comparison is on the subset of epochs where both \citet{clements21}
\end{table*}
...
\bottomrule
\end{tabular}
\tablecomments{Traditional pipeline: AstroPy v5.3 \texttt{photutils}
\end{table*}
