...
Table~\ref{tab:abl_cross} summarizes the contribution of each major framework component, compiled from the ablation results reported across \S\ref{sec:ablation}--\S\ref{sec:ood}. These numbers are drawn from experiments already described in their respective sections; no new experiments were conducted for this summary. The table serves to provide a single-point reference for reviewers evaluating the incremental contribution of each architectural choice.
