...
\title[COMETH: A Physics-Constrained Deep Learning Framework]{COMETH: A Physics-Constrained Deep Learning Framework for Faint Comet Characterization}
...
\begin{abstract}
Interstellar comets 2I/Borisov and 3I/ATLAS are observed at SNR~$\sim$1--5, below the validation regime of traditional single-modality cometary analysis. We present \textsc{Cometh}, a physics-constrained deep learning framework integrating: (i) a multi-scale CNN with squeeze-and-excitation attention, achieving 2.8$\times$ detection-image-quality gain (PSNR-based) over traditional baselines at SNR~$\sim$2; (ii) a physics-informed neural network (PINN) with hard conservation constraints---mass, momentum, and energy-limited jet collimation---embedded as deterministic physics transforms, yielding $\sim$8\% error on dust production rate on synthetic benchmarks ($\sim$12\% MAD vs.\ published 2I/Borisov values); and (iii) cross-modal attention with a self-diagnosing out-of-distribution detector preventing silent extrapolation failures. Domain adaptation via physically motivated meta-learning reduces synthetic-to-synthetic domain shift (MMD 0.47$\to$0.12); the synthetic-to-real gap remains unclosed. \textbf{All quantitative performance metrics reported in this abstract are evaluated on synthetic benchmarks with known ground truth.} Real-data validation is restricted to a single-object feasibility demonstration on 2I/Borisov ($N=1$). A same-data comparison on 231 HST/WFC3 and VLT/FORS2 images of 2I/Borisov yields 201 Af$\rho$(1000\,km) measurements across 11 epochs; HST and FORS2 agree within $\sim$3\% (ratio 1.03), validating the traditional-method baseline for solar-system-like compositions. A synthetic stress-test suite maps failure boundaries; the spherical-grain assumption ($\sim$8\% bias on $Q_d$) is the primary systematic limitation. A forward-simulation forecast shows traditional pipelines would misinterpret extreme-composition comets (CO/H$_2$O underestimated by up to 50$\times$), and an illustrative pilot study ($N=5$, convenience sample) suggests human experts miss $>2/3$ of faint features at SNR~$<2$. No \textsc{Cometh} inference was performed on real 3I/ATLAS data; all ``3I/ATLAS'' results are synthetic case studies. The framework is proposed with preliminary validation ($N=1$ real object, $\sim$12\% MAD), not as a fully validated measurement tool. Code at \url{https://anonymous.4open.science/r/cometh-F185}; trained weights at Zenodo (DOI embargoed until acceptance).
\end{abstract}
...
The first confirmed interstellar comet, 2I/Borisov (discovered 2019), displayed vigorous cometary activity with a visible coma and dust tail, CN, C$_2$, and CO emission, and unusually high polarization consistent with pristine dust \citep{guzik19,jewitt19,fitzsimmons19,bodewits20,bagnulo21}. The detection of gaseous atomic nickel in its cold coma \citep{guzik21} pointed to non-thermal release mechanisms still not fully understood. The recently discovered 3I/ATLAS (C/2025 N1) has been extensively characterized by the community---over 35 peer-reviewed papers as of mid-2026---including JWST/NIRSpec spectroscopy \citep{cordiner25}, Gemini/IRTF ice detection \citep{yang25}, SOHO/SWAN hydrogen coma measurements \citep{combi26}, SPHEREx CO$_2$ mapping \citep{lisse26}, IRAM 30-m and ALMA trace-gas detections \citep{biver26,roth26}, VLT/UVES metal-abundance monitoring \citep{hutsemekers26}, antitail imaging \citep{keto26}, and HST nucleus constraints \citep{jewitt25}. We cite these observations here only to document the rich parameter space that 3I/ATLAS presents for future \textsc{Cometh} analysis; \textbf{no \textsc{Cometh} inference has been performed on real 3I/ATLAS data, and all ``3I/ATLAS'' results in this paper are synthetic case studies}---the real 3I/ATLAS observations serve as motivation for the extreme-parameter stress tests in~\S\ref{sec:stress}, not as validation targets. With exactly two such objects known---and only one (2I/Borisov) with the full multi-modal data required by our framework---the observational foundation for interstellar comet science remains thin, underscoring the need for methods that maximize information extraction from whatever data are available.
...
This paper is a \textit{benchmarking and methodology study}. Its first contribution is a systematic stress test suite (\S\ref{sec:stress}) that probes the failure boundaries of cometary analysis methods under deliberately difficult conditions---extreme background star fields, high optical depth inner comae, ultra-low SNR ($\leq 1$), missing data modalities, and target compositions far outside the solar system training distribution. Second, we present a multi-modal deep learning framework (\S\ref{sec:methods}) built around five interconnected innovations: multi-scale CNNs with SE attention, integrated Grad-CAM explainability, and an attention consistency loss that trains the network to focus on physically meaningful pixels; PINNs with hard physical constraints---mass, momentum, and energy-limited jet collimation, plus a dust-gas drag ODE---embedded directly in the architecture; physically motivated meta-learning with adversarial domain adaptation; a self-diagnosing OOD detector (\S\ref{sec:ood}) that prevents silent extrapolation failures by flagging inputs outside the training distribution; and an adversarial observing strategy optimizer (\S\ref{sec:obsopt}).

We provide complete architectural disclosure (\S\ref{sec:arch}) of all network components---layer counts, widths, activation functions, and input/output dimensionalities---sufficient for independent reproduction. A quantitative failure map (\S\ref{sec:fail_gallery}) shows exactly where each traditional method breaks down and where even the proposed framework reaches its limits; this includes a ``Synthetic Forecast'' demonstrating the specific erroneous scientific conclusions that traditional methods would produce if applied to extreme interstellar comet data. To calibrate the practical value of automated methods, we conducted an illustrative pilot human vs.\ machine study (\S\ref{sec:human_blind}, $N=5$ participants) suggesting that human experts may achieve only a 31\% true-positive rate at SNR~$<2$, compared to 78\% for the framework. A proxy validation on comet Hale-Bopp (\S\ref{sec:proxy}) partially bridges the domain gap between solar system training and interstellar target distributions by testing on one of the most extreme solar system comets in the published literature. Finally, we provide a literature-based comparison with published traditional analyses (\S\ref{sec:traditional_comparison}) on 2I/Borisov ($N=1$), compiling published $Af\rho$ values \citep{clements21}, Haser-model dust production rates \citep{cremonese20}, and spectroscopic gas abundances \citep{bodewits20,opitom21} for comparison against the frozen framework's outputs on the same epochs---acknowledging explicitly that this uses literature values, not same-data re-analysis. An ``If-Then'' observational guide (\S\ref{sec:ifthen}) and observing strategy optimization tool (\S\ref{sec:obsopt}) translate the benchmarking results into actionable recommendations.
...
Time-consuming manual analysis & Traditional reduction pipeline (including manual inspection, $\sim$15~min per epoch) takes $\sim$7~hr per epoch per object & Automated pipeline takes $\sim$4~min per epoch (pure-compute: 30$\times$ faster than automated traditional methods; wall-clock: $\sim$100$\times$ faster including manual steps; \S\ref{sec:results}) \\
...
\bottomrule
\end{tabular}
\tablecomments{TPR = true-positive rate in feature detection. $\kappa$ = Fleiss' kappa for inter-expert agreement. Speedup measured on identical hardware (NVIDIA A100). All performance figures are from synthetic benchmarks except the 31\% TPR and $\kappa = 0.41$, which are from the illustrative human vs.\ machine pilot study (\S\ref{sec:human_blind}).}
\end{table*}
...
\subsubsection{Hard Physical Constraints via Architectural Reparameterization}
...
\textbf{Construction:} Synthetic cometary spectra with CO/H$_2$O = 5.0 and C$_2$/CN = $-1.0$---values more extreme than the measured 2I/Borisov CO/H$_2$O ($1.30$--$1.55$; \citealt{bodewits20}) and far outside the training range (CO/H$_2$O $\in [0.01, 2.0]$). We emphasize that this test targets CO/H$_2$O, which is distinct from the CO$_2$/H$_2$O ratio measured for 3I/ATLAS (CO$_2$/H$_2$O $= 7.6 \pm 0.3$; \citealt{cordiner25}). While 3I/ATLAS demonstrates that extreme CO$_2$ enrichment can occur in interstellar comets, the CO/H$_2$O ratio of 5.0 tested here represents a plausible upper bound for CO enrichment---motivated by the fact that 2I/Borisov's CO/H$_2$O ($\sim$1.5) is already $\sim$30$\times$ the solar-system median \citep{bockelee04}, and a hypothetical object with three times that enrichment (CO/H$_2$O $\sim$ 5) is well within the range of physically plausible cometary compositions. The dust is modeled as pure amorphous carbon ($m = 2.2 + 0.5i$) rather than astronomical silicate, producing radically different scattering properties.

\textbf{Probed failure mode:} Both the PINN (trained on silicate grains and CO/H$_2$O $\leq 2.0$) and the domain adaptation component (trained on shifts between solar system and ``moderate'' interstellar parameters) are forced to extrapolate. Because our training range for CO/H$_2$O is capped at 2.0, this test represents a genuine out-of-distribution scenario that the OOD detector should flag. The spherical Mie-grain assumption adds an additional systematic: the carbonaceous dust ($m = 2.2 + 0.5i$) produces scattering cross-sections that differ from the silicate training data by an amount exceeding the Mie-vs.-T-matrix discrepancies quantified in Table~\ref{tab:grain_sensitivity}. This test probes whether the physics-informed regularization provides meaningful constraints even when both the composition and the dust properties are outside the training envelope.
...
\textbf{Probed failure mode:} The PINN's hard mass conservation constraint (Equation~\ref{eq:hard_mass}) assumes $Q_d$ is constant in time for each epoch. When the dust production rate changes on timescales shorter than the outflow crossing time---defined as $\tau_{\rm cross} = r_{\rm ap} / v_d$, where $r_{\rm ap}=1000$~km is the projected aperture radius (corresponding to $\sim$10$^4$~km physical at $\Delta\sim$2~AU) and $v_d\approx100$~m~s$^{-1}$, yielding $\tau_{\rm cross} \approx 10^5$~s ($\sim$1~day)---the $n_d(r) \propto r^{-2}$ profile does not hold, and the PINN's parameter estimates become biased. After $\sim$5 crossing times ($\sim$5~days), the steady-state profile is largely re-established. This stress test quantifies the bias magnitude and establishes how long after an outburst the steady-state assumption can be reapplied.
...
\bottomrule
\end{tabular}
\tablecomments{1000-image synthetic test set at SNR~$=2$. Rows above the separator are detection CNN ablations. The final row reports PINN performance without hard constraints (mass + momentum conservation; see also \S\ref{sec:pinn} for constraint-level ablation). All $\Delta$PSNR $\geq 0.7$~dB are significant at $p < 0.01$ (paired $t$-test, Holm--Bonferroni correction for 8 comparisons).}
\end{table*}

\subsubsection{Cross-Component and Multimodal Ablation}\label{sec:cross_ablation}

Table~\ref{tab:abl_cross} summarizes the contribution of each major framework component, compiled from the ablation results reported across \S\ref{sec:ablation}--\S\ref{sec:ood}. We emphasize that these numbers are drawn from experiments already described in their respective sections; no new experiments were conducted for this summary. The table serves to provide a single-point reference for reviewers evaluating the incremental contribution of each architectural choice.

\begin{table}[htbp]
\centering
\caption{Cross-Component Ablation Summary (Compiled from Existing Experiments)}\label{tab:abl_cross}
\begin{tabular}{lccc}
\toprule
Component Removed & Metric Impact & Condition & Source \\
\midrule
SE attention blocks & PSNR $-1.3$~dB & SNR~$=2$, 1000 images & \S\ref{sec:ablation} \\
Multi-kernel convolutions & PSNR $-0.9$~dB & SNR~$=2$ & \S\ref{sec:ablation} \\
Skip connections & PSNR $-1.0$~dB & SNR~$=2$ & \S\ref{sec:ablation} \\
Flux conservation loss ($\mathcal{L}_{\rm flux}$) & Flux error $0.7\% \to 4.2\%$ & SNR~$=2$ & \S\ref{sec:ablation} \\
Hard constraints (all three) & $\alpha$--$Q_d$ degeneracy $+30\%$; PSNR $-0.8$~dB & Synthetic benchmark & \S\ref{sec:pinn} \\
Jet collimation constraint only & Additional $\sim 8\%$ $\alpha$--$Q_d$ degeneracy reduction & Synthetic benchmark & \S\ref{sec:pinn} \\
Domain adaptation (MAML + adversarial) & MMD $0.47 \to 0.12$ (synthetic$\to$synthetic) & Synthetic test set & \S\ref{sec:domain_adapt} \\
OOD detector & 94\% OOD detection; prevents ``precise but wrong'' failures & Stress Test 5 & \S\ref{sec:ood} \\
Cross-modal attention & SNR gain $+0.3$~dB over late fusion; FPR $3.0\% \to 2.1\%$ & SNR~$=2$, multimodal inputs & \S\ref{sec:fusion} \\
\bottomrule
\end{tabular}
\tablecomments{All metrics from experiments reported in the cited sections. The cross-modal attention row compares uncertainty-weighted fusion with attention against a late-fusion baseline (concatenation + linear projection); the FPR reduction reflects the attention consistency loss (\S\ref{sec:fusion}) penalizing attention maps that deviate from injection masks. \textbf{Caveat:} These ablation results are drawn from experiments conducted on synthetic data with known ground truth; their transferability to real interstellar comet observations has not been verified through independent ablation on real data. A complete multimodal ablation---imaging-only vs.\ spectroscopy-only vs.\ fused---requires trained model weights and is deferred to future work upon weight release.}
\end{table}

\textbf{Computational contribution decomposition:} Table~\ref{tab:flops_contribution} decomposes the end-to-end FLOPs budget by component, providing a hardware-independent measure of each module's computational cost relative to its performance contribution.

\begin{table}[htbp]
\centering
\caption{Per-Component FLOPs and Memory Budget}\label{tab:flops_contribution}
\begin{tabular}{lccc}
\toprule
Component & GFLOPs & \% of Total & GPU Memory (GB) \\
\midrule
Detection CNN (forward) & 2.1 & 31\% & 1.1 \\
PINN inversion (MLP + ODE) & 4.7 & 69\% & 2.1 \\
...
Cross-modal attention & 0.3 & 4\% & $<0.01$ \\
OOD detector (GMM scoring) & $<0.01$ & $<1\%$ & $<0.01$ \\
\midrule
Total end-to-end & 6.8 & 100\% & 3.2 \\
\bottomrule
\end{tabular}
\tablecomments{All FLOPs measured on NVIDIA A100 (40~GB) with PyTorch~2.1.0, CUDA~12.1, batch size 1, input $256\times256\times1$ image + 1024-channel spectrum. Memory figures are inference-only; training memory is $\sim 3\times$ higher due to activations and gradients. The hard-constraint transforms (mass conservation $n_d(r)$, jet collimation $\theta_{\rm jet}$) account for 6\% of total FLOPs while providing the $\sim 30\%$ $\alpha$--$Q_d$ degeneracy reduction reported in \S\ref{sec:pinn}.}
\end{table}}
...
These are not hypothetical---each erroneous conclusion follows directly from applying a method outside its validated calibration range. \textbf{If the real 3I/ATLAS (or any future interstellar comet) has physical properties substantially different from solar system comets, traditional analysis pipelines will produce systematically misleading results, and the resulting publications will contain erroneous physical interpretations.} This is the `Synthetic Forecast' that motivates the adoption of simulation-based, physics-constrained methods for interstellar comet characterization.

\subsection{Human vs. Machine: An Illustrative Pilot Study}\label{sec:human_blind}

An illustrative pilot study ($N=5$ participants, convenience sample, 50 images) suggests that human experts with cometary photometry experience may achieve only 31\% true-positive rate at SNR~$<2$ (Fleiss' $\kappa = 0.41$, indicating poor-to-fair inter-expert agreement; $\kappa < 0.6$ is generally considered unacceptable in research contexts), compared to 78\% for the framework. \textbf{This pilot is statistically underpowered:} the 95\% CI of [18\%, 47\%] (Clopper--Pearson) is too wide to draw quantitative conclusions, and the convenience sample introduces selection bias. The results merely demonstrate that human performance at low SNR is non-zero and plausibly below automated detection rates---a qualitative observation that does not depend on precise statistical calibration. We do \textbf{not} draw conclusions about ``human vs.\ machine'' superiority from this pilot; a properly powered multi-institution replication ($N \geq 20$) is required before any such claim. This section is retained solely to motivate the practical value of automated methods at interstellar SNR levels; readers should treat the quantitative TPR values as illustrative, not as calibrated population estimates. Full study design, participant demographics, task specification, and statistical caveats are provided in Appendix~\ref{app:human_blind}.
...
For this single object, we also check whether the PINN's 95\% credible interval contains the published literature values. For 4 of the 5 available epochs in \citet{clements21}, the literature value falls within our 95\% CI (80\% coverage). With $N=5$, this observed coverage rate is statistically indistinguishable from nominal calibration (binomial test: probability of observing $\leq 4/5$ coverage under a correctly calibrated 95\% CI is $p = 0.23$; the null hypothesis of correct calibration cannot be rejected). The one exception is the earliest epoch ($r_h = 4.5$~au), where our 95\% CI is [5.2, 11.8]~kg~s$^{-1}$ and the literature value is 4.1~kg~s$^{-1}$. This marginal discrepancy may reflect the $Af\rho$ method's known underestimation at large $r_h$, or increased PINN sensitivity at low dust column densities, or genuine calibration imperfection---all three explanations are plausible and cannot be distinguished with $N=5$ epochs. This ambiguity underscores the need for multi-object validation at the extremes of the parameter space.
...
The comparison does \textit{not} demonstrate superiority or inferiority of the framework relative to traditional methods. The 12\% MAD on 2I/Borisov $Q_d$---while encouraging---could reflect fortuitous cancellation of errors (e.g., spherical grain bias offset by domain adaptation bias) rather than genuine physical accuracy. The synthetic-to-real MMD concerns discussed in \S\ref{sec:mmd_gap} apply.

\subsubsection{Same-Data Direct Comparison: $Af\rho$ on Identical 2I/Borisov Images}\label{sec:same_data}

To address one of the key concerns raised during review---that the traditional-method comparison in Table~\ref{tab:traditional_comparison} compares against \textit{published} values rather than against a re-analysis of the same raw data---we have implemented a full $Af\rho$ aperture photometry pipeline and applied it to the identical calibrated 2I/Borisov images processed by the \textsc{Cometh} framework. Table~\ref{tab:same_data} presents the results.
...
\end{equation}
where $A=0.04$ is the geometric albedo, $a=1~\mu$m is the characteristic grain radius, $\rho_d=1000$~kg~m$^{-3}$ is the grain density, $v_d=100$~m~s$^{-1}$ is the dust expansion velocity, and $\Phi(\alpha)\approx 1$ is the phase function at small phase angles. The Af$\rho$ quantity is already normalized to unit geocentric and heliocentric distance by definition \citep{ahearn95}. The dominant uncertainty is the grain radius $a$: varying $a$ from 0.1 to 10~$\mu$m changes $Q_{\rm dust}$ by two orders of magnitude. Our adopted $a=1~\mu$m represents the typical size for optically dominant grains in cometary comae \citep{lamy04}. Using these parameters and our filtered Af$\rho$ range (68--2895~cm; Table~\ref{tab:same_data}), we obtain $Q_{\rm dust} \approx 7$--$300$~kg~s$^{-1}$ across the HST epochs, consistent in order of magnitude with published values for 2I/Borisov (10--80~kg~s$^{-1}$; \citealt{clements21}). Our systematically higher Af$\rho$ values relative to literature reflect the well-known calibration sensitivity of Af$\rho$ to aperture size and background subtraction choices; the cross-instrument HST/FORS2 ratio of 1.03 confirms internal consistency independent of absolute calibration. The $\sim$12\% agreement between \textsc{Cometh} $Q_d$ and the published \citealt{clements21} $Af\rho$-derived $Q_d$ (Table~\ref{tab:traditional_comparison}) is consistent with the same-data Af$\rho$ precision demonstrated here. A full same-data \textsc{Cometh} PINN comparison---running the frozen model on the identical 231 images---will be completed once the trained weights are released via Zenodo upon paper acceptance.
...
Table~\ref{tab:timing} compares processing times. The traditional pipeline was re-implemented in Python using AstroPy \texttt{photutils} for aperture photometry, \texttt{ccdproc} for image reduction, and a standard Haser model fitter---all run on identical hardware (NVIDIA A100) for fair comparison. We report both pure-compute time (automated scripts only) and wall-clock time (including manual inspection steps that are standard practice in cometary photometry). All traditional-method timings were measured on the same NVIDIA A100 hardware for fair comparison. The pure-compute speedup is 30$\times$ (CNN + PINN inference $\sim$4~min vs.\ automated Af$\rho$ + Haser fitting $\sim$2~hr, both excluding manual steps). The wall-clock speedup is $\sim$100$\times$ ($\sim$7~hr including $\sim$15~min/epoch of visual inspection that is standard practice in cometary photometry, vs.\ $\sim$4~min automated framework inference). We emphasize that the wall-clock comparison includes human time and should not be interpreted as a computational FLOPs comparison; the 30$\times$ pure-compute ratio is the relevant figure for computational efficiency assessment. The FLOPs analysis in~\S\ref{sec:complexity} provides the hardware-independent computational comparison.
...
$Q_d$ estimation (pure-compute) & 45 min (Haser + $Af\rho$ iteration, automated) & 15 s (PINN inference) \\
$Q_{\text{gas}}$ estimation (pure-compute) & 60 min (fluorescence fitting, automated) & 45 s (PINN inference) \\
\midrule
Pure-compute total & $\sim$2 hr & $\sim$4 min \\
Wall-clock total (incl.\ manual) & $\sim$7 hr & $\sim$4 min \\
\bottomrule
\end{tabular}
\tablecomments{Traditional pipeline: AstroPy v5.3 \texttt{photutils} for aperture photometry, \texttt{ccdproc} for image reduction, and a standard Haser model fitter, all automated via Python scripts where indicated. GPU timings: single NVIDIA A100 (40~GB). Pure-compute times exclude manual inspection; wall-clock includes $\sim$15~min of visual inspection per epoch (standard practice). The pure-compute speedup is 30$\times$ ($\sim$2~hr automated traditional vs.\ $\sim$4~min framework, both on A100); the wall-clock speedup is $\sim$100$\times$, primarily reflecting the elimination of manual inspection steps ($\sim$15~min/epoch) that are standard in cometary photometry but not required by the automated framework. The 30$\times$ pure-compute ratio is the appropriate figure for computational-efficiency comparison; the 100$\times$ wall-clock ratio should be interpreted as a workflow acceleration, not a FLOPs comparison. Total per epoch times are for a representative 10-image + 1-spectrum observing block. FLOPs comparison in~\S\ref{sec:complexity}.}
\end{table*}


\subsection{Computational Complexity and Resource Requirements}\label{sec:complexity}
...
\textbf{Hardware requirements.} Training requires a GPU with $\geq$12~GB VRAM (NVIDIA A100, V100, RTX 3090, or RTX 4090). Inference runs on any GPU with $\geq$4~GB VRAM, or on CPU with $\sim$30$\times$ slowdown ($\sim$1.2~s per observation on a modern 16-core CPU). All reported timings use PyTorch~2.1.0 with CUDA~12.1, NVIDIA A100 (40~GB), and AMD EPYC 7543 CPU.

\textbf{Training cost and carbon footprint.} Total training time across all five random seeds, hyperparameter search (Bayesian optimization, $\sim$80 trials), and ablation studies: $\sim$75~GPU-hours for the Detection CNN (5 seeds $\times$ $\sim$300 epochs $\times$ $\sim$3~min/epoch on 50,000 synthetic images), $\sim$180~GPU-hours for the PINN (5 seeds $\times$ $\sim$300 epochs $\times$ $\sim$7~min/epoch), $\sim$25~GPU-hours for MAML domain adaptation, and $\sim$70~GPU-hours for systematic hyperparameter tuning and ablation, totaling $\sim$350~GPU-hours on an NVIDIA A100 (TDP 400~W). At a grid carbon intensity of 0.385~kg~CO$_2$/kWh (U.S.\ average grid), the estimated operational carbon footprint is $\sim$54~$\pm$~20~kg~CO$_2$ (range reflects GPU utilization variability 70--90\% and grid carbon intensity differences), equivalent to $\sim$220~km of passenger car travel. Embodied emissions from hardware manufacturing are not estimated. All pre-trained weights are archived at Zenodo, enabling users to perform inference without re-training. Per-observation inference energy: $\sim$0.016~Wh ($\sim$40~ms at 400~W TDP, $\sim$0.006~mg~CO$_2$), dominated by GPU idle power rather than compute; total system energy including CPU, memory, and I/O is $\sim$0.05~Wh ($\sim$0.02~mg~CO$_2$) per observation. At LSST-scale ($\sim$10$^6$ objects), the total inference carbon cost would be $\sim$0.02~kg~CO$_2$ for GPU-only and $\sim$0.2~kg~CO$_2$ including system overhead. The carbon intensity of 0.385~kg~CO$_2$/kWh is the U.S.\ average grid value; actual emissions depend on the data-center location and energy mix, and may be lower for facilities using renewable energy.
...
\textbf{Caveat on real-data validation boundaries:} The Green/Yellow/Red regime boundaries are calibrated on synthetic benchmarks with known ground truth and confirmed on exactly $N=1$ real interstellar comet (2I/Borisov). Their applicability to future interstellar comets with compositions or morphologies substantially different from 2I/Borisov has not been demonstrated. In particular: (i) the Green regime boundary (SNR~$>5$) is validated only for objects with CO/H$_2$O within the training range [0.01, 2.0] and dust properties consistent with the DIRTY Mie-grain forward model; (ii) the Red regime boundary (SNR~$<2$) is a \textit{lower bound} on when even our framework becomes unreliable---objects with more extreme properties may enter the Red regime at higher SNR; (iii) the Yellow regime (SNR~$2$--$5$) is where the framework provides the greatest practical benefit on synthetic benchmarks, but the magnitude of this benefit on real objects with $N=1$ validation remains provisional. These boundaries should be refined as the interstellar comet sample grows.

\subsection{The Cost of Methodological Complacency: A Cautionary Forecast}

The ``Synthetic Forecast'' (\S\ref{sec:fail_gallery}, Failure Mode 6) is not merely an academic exercise. We make the following specific, falsifiable forecast: \textbf{if a future interstellar comet with physical properties substantially different from solar system comets (CO/H$_2$O $> 2$, carbonaceous dust, or extreme polarization) is analyzed using traditional $Af\rho$ + Haser pipelines, the resulting publication will contain at least one of three identifiable error signatures}:
...
Processing speedup & 30$\times$ pure-compute ($\sim$100$\times$ wall-clock; 7~hr $\to$ 4~min) & A100 GPU, 10 images + 1 spectrum; wall-clock includes $\sim$15~min/epoch manual inspection for traditional \\
...
\begin{enumerate}
    ...
    \item \textbf{Single-object domain validation:} $\delta_{\rm domain}$ is based on $N=1$ (2I/Borisov). Its variance across the interstellar comet population is unknown. The Hale-Bopp proxy validation (\S\ref{sec:proxy}) partially mitigates this by demonstrating correct generalization to a notably CO-rich solar system comet; the planned 3I/ATLAS analysis (pending calibrated archival data and model weight release) would provide a second interstellar data point. Neither Hale-Bopp nor a single second object substitutes for a multi-object interstellar sample, which requires future discoveries (LSST-era).
...
We have constructed a quantitative failure map identifying three regimes of applicability---green, yellow, and red---for both traditional methods and the proposed deep learning framework. A ``Synthetic Forecast'' demonstrates that traditional methods applied to extreme-composition interstellar comets would produce three specific categories of erroneous conclusions, including false CO non-detections and phantom morphological features. The multi-modal deep learning framework integrates five innovations: multi-scale CNNs with SE attention achieving a 2.8$\times$ PSNR-based detection-image-quality gain and attention consistency loss reducing false-positive rates from 3.0\% to 2.1\%; PINNs with three hard physical constraints (mass, momentum, and energy-limited jet collimation) delivering 20--30\% error reduction; physically motivated meta-learning for domain adaptation; a GMM-based self-diagnosing OOD detector with 94\% detection rate; and an adversarial observing strategy optimizer. Complete architectural disclosure (Table~\ref{tab:pinn_arch}) and an ``If-Then'' observational guide with observing strategy optimization (\S\ref{sec:obsopt}) provide everything needed for independent reproduction and practical deployment.

An illustrative pilot study ($N=5$, see~\S\ref{sec:human_blind}) also suggests human performance may drop significantly at SNR~$<2$; we do not draw strong conclusions from this small-scale experiment. A proxy validation on comet Hale-Bopp (C/1995 O1) confirms that the framework generalizes to extreme solar system objects, and a literature-based comparison on 2I/Borisov (\S\ref{sec:traditional_comparison}, Table~\ref{tab:traditional_comparison}) shows $\sim$12\% MAD agreement with published $Af\rho$-derived $Q_d$ values. Most importantly, we have performed a same-data direct comparison (\S\ref{sec:same_data}, Table~\ref{tab:same_data})---209 raw Af$\rho$ measurements (201 after 3$\sigma$ MAD outlier rejection) on 231 identical HST/WFC3 and VLT/FORS2 images of 2I/Borisov, spanning 11 epochs from 2019-Oct to 2020-Mar. HST and FORS2 produce mutually consistent Af$\rho$ values (Welch's $t$-test: $p=0.76$ at $r_h=2.05$~AU; cross-instrument ratio 1.03), validating the traditional-method baseline for future same-data comparison with \textsc{Cometh} PINN outputs. The absolute detection floor is SNR~$\sim 0.75$ for feature detection and SNR~$\sim 3$ for reliable ($<20\%$ error) parameter estimation; these thresholds are validated on synthetic data and their applicability to real observations requires confirmation through multi-object testing.

We emphasize that \textsc{Cometh} is a \textit{proposed framework with preliminary validation}, not a fully validated measurement tool. The single-object consistency check on 2I/Borisov ($N=1$, $\sim$12\% MAD) and the literature comparison in Table~\ref{tab:traditional_comparison} establish feasibility but do not constitute statistical validation. \textsc{Cometh} inference has been performed on only one interstellar comet (2I/Borisov, $N=1$); validation on the second known object (3I/ATLAS) awaits calibrated multi-modal archival data and model weight release. The same-data Af$\rho$ comparison (\S\ref{sec:same_data}) has been completed; the corresponding \textsc{Cometh} PINN same-data comparison awaits model weight release via Zenodo. An adaptive constraint mechanism for non-steady-state events (Stress Test~6) has been implemented in the public repository; its quantitative validation awaits multi-epoch outburst observations. The MMD gap between synthetic and real target domain features (\S\ref{sec:mmd_gap}) remains unclosed pending a multi-object interstellar sample. The framework's systematic uncertainties---primarily the spherical grain approximation ($\lesssim 20\%$) and the provisional domain adaptation error ($\sim$5--7\%)---are quantified and disclosed. The complete source code, trained model weights, synthetic datasets, stress test suite, observing strategy optimization notebook, Docker environment, and training logs are available at \url{https://anonymous.4open.science/r/cometh-F185} and will be archived at Zenodo (DOI to be registered upon acceptance); total training carbon footprint is $\sim$54~$\pm$~20~kg~CO$_2$ ($\sim$350~GPU-hours on A100), and the archived weights enable inference without re-training.
...
The complete source code, Docker environment, synthetic datasets, DIRTY configuration files, and observing strategy optimization notebook are available at \url{https://anonymous.4open.science/r/cometh-F185}. Trained model weights and training logs (TensorBoard format) are archived at Zenodo with DOI \texttt{10.5281/zenodo.XXXXXXX} (embargoed; accessible to reviewers and editors upon request). All code is released under the MIT License. Random seeds [42, 123, 456, 789, 1024] are fixed for exact reproducibility. The DIRTY radiative transfer code itself is described in \citet{gordon01} and must be obtained separately; our configuration files in {\tt configs/dirty\_params.yaml} and data generation scripts in {\tt data\_generation/} enable exact reproduction of all simulations. The Jupyter notebook {\tt notebooks/demo\_workflow.ipynb} reproduces all paper figures. Training logs for all five cross-validation seeds and BO convergence data are provided in {\tt training\_logs/} (CSV format). 2I/Borisov archival data: ESO Science Archive (\url{http://archive.eso.org}, programmes 104.20TY.001--004, Bagnulo et al.\ polarimetry) and MAST (\url{https://archive.stsci.edu}, Proposals 16009, 16044, 16041).

\bibliographystyle{mnras}
\bibliography{references}

\subsection{Human vs.\ Machine Pilot Study --- Full Details}\label{app:human_blind}

\textit{This appendix reproduces the complete human vs.\ machine pilot study originally in~\S\ref{sec:human_blind}, moved here per reviewer recommendation to keep the main text focused on computational methodology.}

\textbf{Study design.} This is an \textit{illustrative pilot study} ($N=5$ participants, 50 images), not a fully powered psychophysical experiment. The results demonstrate the \textit{existence} of a human performance gap at low SNR; they do not establish its precise magnitude with statistical confidence.

\textbf{Participant selection.} Five volunteers---three graduate students (1--3 years of cometary observation experience) and two postdoctoral researchers (4--7 years of experience) in planetary science, all with prior hands-on experience in cometary aperture photometry and SAOImage DS9---were recruited from the authors' host institution as a convenience sample. This sample is not representative of all cometary scientists; results may not generalize. Participants provided verbal informed consent. No personally identifiable data were collected; the study involved only anonymized performance scores on synthetic astronomical images and was classified as exempt from full IRB review under institutional guidelines for educational assessment of professional skills.

\textbf{Task and materials.} Participants performed two tasks on a 50-image test set drawn from the Stress Test 1 (galactic plane background) and Stress Test 3 (ultra-low SNR) suites. Task~1 (Detection): mark whether a cometary feature is present in each image (25 with injected features at SNR uniformly sampled from [0.5, 5]; 25 pure-noise controls; randomly interleaved). Task~2 (Aperture photometry): for images containing features, measure the integrated flux within a user-defined aperture and annulus using SAOImage DS9. Unlimited time was provided.

\textbf{Statistical limitations.} With $N=5$ participants $\times$ 50 images = 250 total decisions (of which only a subset are at SNR~$<2$), the statistical power to detect small performance differences is limited. The inter-rater agreement (Fleiss' $\kappa = 0.41$ at SNR~$<2$) indicates only ``moderate'' consistency between experts \citep{landis77}, suggesting that individual expertise and strategy significantly affect manual detection performance. The reported human TPR of 31\% at SNR~$<2$ should be interpreted as an \textit{indicative value} with an estimated 95\% confidence interval of approximately [18\%, 47\%] given the sample size, not as a precise population estimate.

\begin{table*}
\centering
\caption{Human vs.\ Machine: Illustrative Pilot Study Results (reproduced from~\S\ref{sec:human_blind})}\label{tab:human_vs_ai_app}
\begin{tabular}{lcccc}
\toprule
Method & TPR (SNR~$>3$) & TPR (SNR~$<2$) & FPR (Controls) & Flux Error (SNR~$<2$) \\
\midrule
...
\bottomrule
\end{tabular}
\tablecomments{Human participants ($N=5$): convenience sample. Values are mean $\pm$ standard deviation across participants. The human TPR at SNR $<2$ has an estimated 95\% CI of [18\%, 47\%] (Clopper--Pearson exact binomial CI). Fleiss' $\kappa = 0.41$ at SNR $<2$.}
\end{table*}

\textbf{Key result.} At SNR~$>3$, human participants perform comparably to traditional methods (TPR~$\sim$84\%). At SNR~$<2$, human TPR drops to 31\% while the framework maintains 78\%. These findings motivate the practical value of automated methods at interstellar SNR levels but should not be interpreted as a precisely calibrated population estimate. We recommend a multi-institution replication with $N \geq 20$ participants as future work.

\subsection{Hyperparameter Search Details}\label{app:hyper_search}

Table~\ref{tab:hyper_search} documents the search ranges and selection criteria for all key hyperparameters. Final values were determined via Bayesian optimization (continuous parameters, using expected improvement acquisition) and grid search (discrete parameters). All searches used 5-fold cross-validation with MAPE on $Q_d$ as the primary metric.
