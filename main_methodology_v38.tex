...
\textbf{Training cost and carbon footprint.} Total training across all seeds, hyperparameter search, and ablation consumed $\sim$350~GPU-hours on an NVIDIA A100, corresponding to an estimated operational carbon footprint of order $\sim$50~kg~CO$_2$ (equivalent to $\sim$200~km of passenger car travel). Embodied hardware emissions are not estimated. Trained weights will be archived at Zenodo upon acceptance, eliminating the need for re-training for inference-only use. Per-observation inference energy is negligible ($\sim$0.05~Wh, $<0.02$~mg~CO$_2$). \textbf{Reproducibility verification} (re-running training with fixed seeds) would incur the full $\sim$50~kg~CO$_2$ cost; we provide the trained weights so that reproducibility can be checked via inference comparison without mandatory re-training, but acknowledge that full end-to-end reproducibility verification carries this carbon cost.

\textbf{Ethical note on computational cost:} The $\sim$50~kg~CO$_2$ training cost is substantial for a methodology targeting a population of $N=2$ known objects. We justify this investment on three grounds: (1) the pre-trained weights eliminate re-training for inference-only use ($<0.02$~mg~CO$_2$ per observation), though full reproducibility verification via re-training would incur the $\sim$50~kg~CO$_2$ cost; (2) the six-test stress suite and Af$\rho$ pipeline are immediately reusable without GPU training, providing carbon-free benchmarking; (3) the framework applies to faint solar system comets ($N \gg 100$), amortizing the one-time training cost over a larger population. We encourage inference use of pre-trained weights where possible; re-training is necessary only for hyperparameter modifications or full verification. \textbf{If multiple groups independently re-train for verification, the aggregate carbon cost scales linearly with the number of groups---a cost externalized to the community.} The 50~kg~CO$_2$ is the methodology developer's cost, not the community's total cost if the protocol is widely adopted. Users should weigh this cost against the scientific benefit before re-training.

\textbf{Cost-benefit context:} The $\sim$54~kg~CO$_2$ training cost must be weighed against the framework's demonstrated benefit. On synthetic benchmarks, the framework provides a 10--20 percentage-point error reduction over traditional methods in the Yellow regime (SNR 2--5), but \textit{no accuracy advantage in the Green regime} (SNR $>$5, \S\ref{sec:failure_map}). For a population currently consisting of $N=2$ known interstellar comets, the carbon investment is substantial relative to the number of objects that can benefit. This cost-benefit ratio improves if: (a) LSST or other surveys increase the known interstellar comet population to $N \gg 10$; (b) the framework is applied to faint solar system comets observed at large heliocentric distances, which share similar SNR challenges and are far more numerous; or (c) pre-trained weights reduce marginal cost to inference-only for operational use, though reproducibility verification via re-training carries the full $\sim$50~kg~CO$_2$. We provide the trained weights specifically to enable option (c). The current carbon investment is the methodology developer's one-time cost; community-wide re-training for verification would multiply this cost. Users should default to pre-trained weights for inference and re-train only when necessary.
