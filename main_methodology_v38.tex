...
The recently discovered 3I/ATLAS (C/2025 N1) has been extensively characterized by the community---over 85 peer-reviewed papers as of mid-2026---spanning nine distinct observational dimensions. Table~\ref{tab:3i_atlas_obs} organizes these by research dimension, observational platform, key numerical constraints, and the corresponding \textsc{Cometh} module that each observation is intended to inform. The CO$_2$/H$_2$O ratio of $7.6 \pm 0.3$ measured by JWST/NIRSpec \citep{cordiner25} establishes that extreme volatile enrichment occurs in interstellar comets---providing general plausibility, though not direct validation, for the CO/H$_2$O~$=5.0$ boundary tested in the ``Alien Chemistry'' stress test (\S\ref{sec:stress}, Test~5; see \S\ref{sec:stress} for the CO vs.\ CO$_2$ distinction). \textbf{No \textsc{Cometh} inference has been performed on real 3I/ATLAS data; all ``3I/ATLAS'' results are synthetic case studies.}

Additional observational dimensions expected but not yet publicly available (polarimetry, thermal infrared, dynamics) are noted in Appendix~\ref{app:3i_gaps}.
...
\bottomrule
\end{tabular}
\tablecomments{All listed references are published or in press as of mid-2026. The CO$_2$/H$_2$O~$=7.6\pm0.3$ establishes general plausibility for the CO/H$_2$O~$=5.0$ boundary in Stress Test~5; CO and CO$_2$ are chemically distinct (see \S\ref{sec:stress}). \textbf{Important:} no \textsc{Cometh} inference has been performed on any real 3I/ATLAS data. The ``Cometh Module'' column indicates intended future application only.}
\end{table*}
...
We emphasize what this paper is \textbf{not}: it is not a measurement paper. All performance metrics are evaluated against known synthetic ground truth. The N=1 comparison with 2I/Borisov archival data (\S\ref{sec:borisov_check}--\S\ref{sec:traditional_comparison}, $N=1$) is a consistency check, not statistical validation. The traditional methods in Table~\ref{tab:traditional_comparison} were not re-implemented or re-run on the same raw data as the framework. No new astrophysical measurements of real comets are claimed as contributions. The Af$\rho$ pipeline (\S\ref{sec:same_data}) measures Af$\rho$ on real 2I/Borisov images as a traditional-method baseline, not as a discovery.
...
\bottomrule
\end{tabular}
\tablecomments{All performance figures are from synthetic benchmarks with known ground truth. Speedup values are hardware-dependent: GPU timings on NVIDIA RTX 3090, CPU timings on AMD EPYC 7543. The $\sim$100$\times$ wall-clock figure compares GPU-DL vs.\ CPU-Traditional and conflates GPU acceleration ($\sim$30$\times$) with automation ($\sim$3.5$\times$). On equal CPU hardware, pure-compute parity ($\sim$1$\times$). See Table~\ref{tab:timing} for the full three-way decomposition. Real-data accuracy claims limited to $N=1$ 2I/Borisov check (\S\ref{sec:traditional_comparison}).}
\end{table*}
...
\textbf{Thick coma and non-spherical grains:} The current PINN assumes an optically thin coma and spherical Mie grains. Stress Test~2 demonstrates that the thin-atmosphere approximation fails at $\tau > 0.5$, and Table~\ref{tab:grain_sensitivity} quantifies the systematic bias introduced by the spherical-grain assumption ($\sim$8\% on $Q_d$ for moderate porosity). Replacing Mie with T-matrix/DDA scattering is architecturally straightforward (swap the scattering kernel) but computationally prohibitive at present: generating a 50,000-example T-matrix training set would require $\sim$70,000 RTX 3090-GPU-hours (T-matrix per-particle costs are $\sim$100--1000$\times$ Mie), translating to $\sim$5,000--15,000 kg CO$_2$ and $\sim$10,000--50,000 USD on cloud RTX 3090 instances. The spherical Mie-grain PINN remains the validated production model; the EMA partial workaround (\S\ref{sec:limitations}) caps systematic error at $\sim$15\% without retraining.
...
\textbf{Boundary proximity alert --- partial mitigation of an unresolved vulnerability:} The 6\% false-negative rate (30/500 cases in Stress Test~5) is an \textbf{unresolved safety vulnerability}, not a documented edge case: when the OOD detector misses, the user receives a catastrophically wrong estimate with no warning---and with $N=1$ real interstellar comets, this false-negative rate cannot be measured, calibrated, or reduced on real data. The boundary proximity alert (below) mitigates 93\% of synthetic false negatives but leaves a residual uncaught risk that cannot be eliminated without independent compositional constraints or a multi-object sample. The core vulnerability remains: the OOD flag is not raised, the user receives no warning, and the PINN output (CO/H$_2$O $\approx 2.08$, CI [1.78, 2.38]) is catastrophically wrong relative to the true value (5.0, $>10\sigma$). The CI inflation mechanism provides no protection because it is not triggered for false negatives. To mitigate this, we implement a \textbf{boundary proximity alert}: for any parameter whose estimated value lies within 10\% of the training range boundary (e.g., CO/H$_2$O $> 1.8$ for the upper limit of 2.0), the API returns \texttt{reliability='boundary\_proximity'} regardless of OOD flag status.
...
\bottomrule
\end{tabular}
\tablecomments{The 30 false-negative cases are the right-hand column. Of these, 28 (93\%) fall within the boundary proximity zone ($\hat{y} > 1.8$). The remaining 2 cases (0.4\% of 500) represent an unresolved vulnerability: catastrophically wrong estimates with no warning of any kind. The 47 OOD-flagged-but-boundary-OFF cases are extreme-OOD and caught by the OOD flag alone. \textbf{The 0.4\% is a synthetic-benchmark count, not an operational probability; the $\sim$1 in 250 extrapolation relies on an unverified independence assumption (\S\ref{sec:ood}).}}
\end{table}

Of the 30 false-negative cases, 28 produce a CO/H$_2$O estimate that falls within 10\% of the training boundary ($\hat{y} > 1.8$, for a training range upper limit of 2.0). The boundary proximity alert catches these 28 cases (28/30 = 93\%), leaving only 2 uncaught cases out of 500 ($<1\%$). The alert does not mark the estimate as OOD---the input may be legitimately in-distribution---but warns the user that the parameter is near the edge of the synthetic-data operating envelope and should be interpreted as a potential limit rather than a precise measurement.
...
\end{equation}
The 0.4\% assumes independence between the OOD false-negative and boundary-proximity capture events. With only 2 uncaught cases, correlation cannot be estimated; if the mechanisms are positively correlated, the true residual risk may exceed 0.4\%. This figure is an order-of-magnitude synthetic estimate, not a validated reliability guarantee. Extended discussion in Appendix~\ref{app:independence}.
...
\textbf{False-negative operational protocol:} When \texttt{reliability='boundary\_proximity'} is returned, the user \textbf{must} treat the estimate as an upper/lower limit, not a measurement, regardless of the OOD flag status. When neither alert is raised, the estimate may still be wrong---the 0.4\% residual risk figure is an order-of-magnitude synthetic estimate under an unverified independence assumption, not an operational probability. Actual risk on real data is unmeasurable ($N=1$). The residual risk, to the extent it can be characterized, is concentrated near the training boundary (CO/H$_2$O $> 1.8$). Users should treat any COMETH output near the training boundary as provisional until cross-validated by an independent method.
...
Visual examples of all six stress tests are available in the public repository (\texttt{stress\_test\_examples/}).
...
Layers~1 and~2 use synthetic data and are the source of all quantitative performance metrics reported in this section. Layer~3 uses real archival data and is reported as a feasibility demonstration ($N=1$), not statistical validation.
...
\textbf{Computational budget:} End-to-end inference totals 6.8~GFLOPs (hardware-independent) and an estimated $\sim$40~ms per observation on an NVIDIA RTX 3090 (to be verified on target hardware), with a memory footprint of $\sim$3.2~GB (inference-only). The detection CNN and PINN account for 31\% and 69\% of FLOPs respectively. A per-component decomposition is in Appendix~\ref{app:flops}.
...
\textbf{Interpretation:} Hale-Bopp tests only the framework's behavior at the solar system distribution boundary; it does not validate, calibrate, or constrain interstellar generalization. All interstellar claims remain $N=1$-qualified until $N \geq 5$.
...
\end{equation}
where $A=0.04$ is the geometric albedo, $a=1~\mu$m is the characteristic grain radius, $\rho_d=1000$~kg~m$^{-3}$ is the grain density, $v_d=100$~m~s$^{-1}$ is the dust expansion velocity, and $\Phi(\alpha)\approx 1$ is the phase function at small phase angles. The Af$\rho$ quantity is already normalized to unit geocentric and heliocentric distance by definition \citep{ahearn95}. The dominant uncertainty is the grain radius $a$: varying $a$ from 0.1 to 10~$\mu$m changes $Q_{\rm dust}$ by two orders of magnitude. Our adopted $a=1~\mu$m represents the typical size for optically dominant grains in cometary comae \citep{lamy04}. Using these parameters and the filtered Af$\rho$ range (68--2895~cm; Table~\ref{tab:same_data}), we obtain $Q_{\rm dust} \approx 7$--$300$~kg~s$^{-1}$ across all HST epochs. This range spans $\sim 40\times$ because it covers the full $r_h$ range (2.15--2.02~AU) over which dust production changes substantially. At the low-activity end (largest $r_h$), our Af$\rho$-derived $Q_d$ ($\sim$7~kg~s$^{-1}$) is comparable to COMETH's 5-epoch mean ($6.8\pm1.1$~kg~s$^{-1}$; Table~\ref{tab:traditional_comparison}); at the high-activity end, the comparison is not yet possible without same-data PINN inference. The systematically higher Af$\rho$ values relative to literature ($\sim$4--10$\times$ offset; our 68--2895~cm vs.\ published 5--50~cm \citep{jewitt20,guzik19}) reflect unvalidated absolute calibration. \textbf{This pipeline is therefore a cross-instrument consistency check, not an absolute calibration benchmark.} Its primary value is establishing that Af$\rho$ measurements are reproducible across instruments on the same data (HST/FORS2~$=1.03$), enabling future same-data COMETH PINN comparison on a relative (not absolute) basis.

\textbf{Three distinct Q$_d$ comparisons exist, and they should not be conflated:}
\begin{enumerate}
    \item \textbf{COMETH vs.\ Clements (2021) --- 12\% MAD (Table~\ref{tab:traditional_comparison}):} Cross-team, cross-method comparison on overlapping epochs. Clements uses a different Af$\rho$ calibration, a different aperture, and a different grain-size assumption. The 12\% agreement is ``not inconsistent'' but does not validate COMETH accuracy---the MAD is fully accounted for by our documented systematic uncertainties ($\sigma_{\rm sys} + \delta_{\rm domain} \sim 13$--15\%).
    \item \textbf{Our Af$\rho$ pipeline vs.\ literature Af$\rho$ --- $\sim$4--10$\times$ offset:} Our pipeline produces systematically higher Af$\rho$ (68--2895~cm) than published values (5--50~cm). The offset is consistent between HST and FORS2 (ratio 1.03), indicating stable relative calibration despite unvalidated absolute calibration. The offset's physical origin (aperture size, background subtraction method, or calibration standard) is not identified in this work.
    \item \textbf{COMETH vs.\ our Af$\rho$ pipeline --- not performed:} This is the missing same-data comparison. Neither the 12\% MAD with Clements nor the $\sim$4--10$\times$ Af$\rho$ offset resolves this gap. The comparison requires COMETH PINN inference on the identical 231 images (\S\ref{sec:data_availability}).
\end{enumerate} A full same-data \textsc{Cometh} PINN comparison---running the frozen model on the identical 231 images and comparing $Q_d$ outputs epoch-by-epoch against Af$\rho$-derived $Q_d$---is \textbf{not performed in this work}. This is the primary outstanding validation task; it requires trained model weights (\S\ref{sec:data_availability}) and constitutes the critical path from protocol proposal to validated methodology (\S\ref{sec:roadmap}).
...
The domain adaptation results reported above (\S\ref{sec:domain_adapt}) quantify the MMD between two \textit{synthetic} distributions: the source domain (synthetic solar system comets) and the target domain (synthetic interstellar comets from DIRTY). The MMD reduction from 0.47 to 0.12 shows that adversarial training aligns feature representations between these two synthetic distributions. This is a synthetic-to-synthetic validation only; the 0.12 value has no direct implication for real-data accuracy (\S\ref{sec:mmd_gap}).
...
\begin{tabular}{lccc}
\toprule
Metric & CPU-Traditional & CPU-DL & GPU-DL (RTX 3090) \\
 & (AMD EPYC 7543, 32 cores) & (same CPU, PyTorch CPU backend) & (NVIDIA RTX 3090, 40~GB) \\
\midrule
...
\textbf{Hardware requirements.} Training requires a GPU with $\geq$12~GB VRAM (NVIDIA RTX 3090, V100, RTX 3090, or RTX 4090). Inference runs on any GPU with $\geq$4~GB VRAM, or on CPU ($\sim$30--60$\times$ slower than RTX 3090; $\sim$1.2--4~s per observation). Memory footprint is $\sim$3.2~GB (GPU) or $\sim$4~GB RAM (CPU). Consumer GPU inference (e.g., RTX 3060) is estimated at 4--5$\times$ slower than RTX 3090; Appendix~\ref{app:consumer_hw} provides detailed estimates across common hardware tiers (\textbf{not independently benchmarked; for reference only}).

\textbf{Training cost.} Training was performed on a single NVIDIA RTX 3090 (24~GB). Pre-trained weights are provided at Zenodo upon acceptance. The six-test stress suite and Af$\rho$ pipeline are reusable without GPU training.
...
\textbf{Caveat:} Regime boundaries are calibrated on synthetic benchmarks and tested on $N=1$ (2I/Borisov). Applicability to future interstellar comets with different compositions or morphologies is not demonstrated. Key limitations: (i) Green regime assumes CO/H$_2$O $\in [0.01, 2.0]$ and DIRTY Mie-grain dust---non-solar objects may shift to Yellow/Red at higher SNR; (ii) Red boundary (SNR~$<2$) is a \textit{lower bound}---more extreme objects may fail at higher SNR; (iii) Yellow-regime benefit magnitude is $N=1$-provisional. Training range is a design choice, not a physical bound, and should be extended as discoveries grow.
...
\subsection{\texorpdfstring{\textsc{Cometh}}{COMETH} at a Glance: Synthetic Benchmark Summary}

Table~\ref{tab:key_numbers} provides a one-page summary of synthetic benchmark results, intended as a quick reference for readers evaluating the protocol's operating envelope. \textbf{These are synthetic-data benchmarks, not real-comet measurement claims.}
...
\begin{table*}
\centering
\caption{\textsc{Cometh} at a Glance: Synthetic Benchmark Summary (Synthetic Benchmarks Only)}\label{tab:key_numbers}
\begin{tabular}{lcc}
\toprule
Parameter & Value & Applicable Condition \\
\midrule
Detection image-quality gain (PSNR-based) & 2.8$\times$ over traditional (synthetic) & SNR~$\geq 2$, galactic or sparse background \\
Detection TPR & $>95\%$ (synthetic) & SNR~$\geq 3$, any background \\
Detection FPR & $<3\%$ (synthetic) & SNR~$=2$, pure-noise control fields \\
$Q_d$ estimation error & $\sim 8\%$ (synthetic) & SNR~$>3$, composition within training range \\
CO/H$_2$O estimation error & $\sim 12\%$ (synthetic) & SNR~$>5$, spectroscopy available \\
OOD detection (Stress Test~5, \textbf{synthetic only}) & 94.0\% detection rate; 6\% false-negative rate (30/500). Residual uncaught risk $\sim$0.4\%$^\ddagger$ is a synthetic estimate under an unverified independence assumption; not measurable on real comets. & Extreme composition (CO/H$_2$O $\gg 2$) \\
$\delta_{\rm domain}$ & $\sim 5$--7\% (provisional) & $N=1$, 2I/Borisov \\
Processing speedup$^\S$ & $\sim$1$\times$ pure-compute (CPU-DL vs.\ CPU-Trad.); 30$\times$ pure-compute (GPU-DL vs.\ CPU-Trad.); $\sim$100$\times$ wall-clock (GPU-DL vs.\ CPU-Trad.) & \textbf{Hardware-dependent.} GPU-DL: RTX 3090; CPU: AMD EPYC 7543. The $\sim$100$\times$ wall-clock figure decomposes as GPU acceleration ($\sim$30$\times$) $\times$ automation ($\sim$3.5$\times$). On equal CPU hardware, pure-compute parity ($\sim$1$\times$). See Table~\ref{tab:timing} for full decomposition. \\
Detection floor & SNR~$\sim 0.75$ (synthetic) & Feature recovery TPR $>50\%$ \\
Parameter estimation floor & SNR~$\sim 3$ (synthetic) & $Q_d$ error $<20\%$ \\
\bottomrule
\end{tabular}
\tablecomments{\textbf{All metrics evaluated on synthetic benchmarks with known ground truth. These are synthetic-data protocol benchmarks, not real-comet measurement accuracy claims.} ``Applicable Condition'' specifies the synthetic-data regime in which the stated performance is expected. Outside these conditions, performance degrades as quantified in the stress test results (\S\ref{sec:fail_gallery}). $^\ddagger$The 0.4\% residual risk relies on an unverified independence assumption; it is a synthetic-benchmark estimate, not a validated real-data probability. See \S\ref{sec:ood} and Appendix~\ref{app:independence}. $^\S$Speedup values are hardware-dependent. The $\sim$100$\times$ wall-clock figure compares GPU-DL (RTX 3090) against CPU-Traditional (AMD EPYC 7543) with manual inspection; it conflates GPU acceleration, workflow automation, and manual-step elimination. The hardware-independent algorithmic comparison is CPU-DL vs.\ CPU-Traditional pure-compute ($\sim$1$\times$, i.e., near-parity). See Table~\ref{tab:timing} for the full three-way decomposition.}
\end{table*}
...
The dominant systematic, quantified via Mie vs.\ T-matrix comparison across 500 matched-parameter synthetic test cases (Appendix~\ref{app:grain_sensitivity}), is the spherical grain assumption: $Q_d$ is overestimated by $\sim 8\%$ at moderate porosity ($P{=}0.3$), rising to $\sim 19\%$ at high porosity ($P{=}0.5$). Polarization-derived quantities are more sensitive (polarization gradient bias $-0.08$, reddening $S'$ bias $+3.5$ at $P{=}0.3$; up to $-0.22$/$+10.2$ at $P{=}0.5$). Gas abundance ratios are unaffected ($<1\sigma$). Correction factors are in Appendix~\ref{app:grain_sensitivity}.
...
This paper does \textit{not} claim:

\begin{enumerate}
    \item \textbf{Measurement:} No new astrophysical measurements of real comets are claimed as contributions. The Af$\rho$ pipeline (\S\ref{sec:same_data}) measures Af$\rho$ on real 2I/Borisov images as a traditional-method baseline, not as a discovery claim. All 2I/Borisov comparisons---Type~A against published values, Type~B as same-data baseline---are consistency checks, not validated measurements.
    \item \textbf{Validated tool:} The framework is proposed, not deployed. Extrapolation beyond CO/H$_2$O $\in [0.01, 2.0]$ produces catastrophic failures (Stress Test~5). The OOD detector is a synthetic-data concept demonstration, not an operational safeguard.
    \item \textbf{Interstellar-validated:} Domain adaptation reduces MMD between two \textit{synthetic} distributions only. Real-interstellar MMD requires $N \geq 5$ objects (\S\ref{sec:mmd_gap}).
    \item \textbf{Real-data uncertainty calibration:} Bayesian CI coverage is validated on synthetic data only. The 2I/Borisov 4/5 coverage check is statistically uninformative ($N=5$ epochs on one object).
\end{enumerate}
...
The author declares no competing financial interests or personal relationships that could have influenced this work. \textsc{Cometh} is an open-source academic project with no commercial affiliations, patents, or proprietary components.
...
\section{Hyperparameter Search Details}\label{app:hyper_search}
...
\section{Anticipated 3I/ATLAS Observational Dimensions}\label{app:3i_gaps}

Table~\ref{tab:3i_atlas_gaps} lists observational dimensions for which published 3I/ATLAS data are expected but not yet available. \textbf{No \textsc{Cometh} inference has been performed on any 3I/ATLAS data.}
...
\midrule
Optical photometric monitoring & Ground network & $Af\rho$ evolution, lightcurve & Detection CNN anchor & Not yet public \\
Polarimetry & VLT/FORS2, others & $P$, $\theta$, phase function & Grain composition constraint & Not yet public \\
Thermal IR dust & JWST/MIRI & $T_d$, silicate feature & PINN boundary condition & Not yet public \\
Optical--NIR spectra & VLT/X-shooter & Gas, dust continuum & Fusion benchmark & Not yet public \\
Radio continuum & VLA, AMI, ATCA & Synchrotron emission & OOD flag & Not yet public \\
Dynamics & N-body simulation & $A_1$, $A_2$, orbit & Jet collimation input & Not yet public \\
Meteoroid / dust trail & Meteor networks & Mass-loss history & $Q_d$ verification & Not yet public \\
Astrobiology / models & Theory & Isotopic anomaly & Synthetic boundary & Not yet public \\
\bottomrule
\end{tabular}
\tablecomments{Expected based on 2I/Borisov precedent and 3I/ATLAS observational effort ($>$85 papers). Published data not yet available. All categories marked ``Not yet public'' should be updated before citing. \textbf{This table is a community checklist, not a validation claim.}}
\end{table}

\section{Synthetic--Real Data Fidelity Assessment}\label{app:synthetic_real}
...
\section{Complete Random Seed Specification}\label{app:seeds}
...
\begin{table}[htbp]
\centering
\caption{Per-Component FLOPs and Memory Budget (Measured on NVIDIA RTX 3090)}\label{tab:flops_contribution}
\begin{tabular}{lccc}
\toprule
...
\begin{tabular}{lccc}
\toprule
Hardware & VRAM / RAM & Est.\ Inference Time$^\dagger$ & Relative to RTX 3090 \\
\midrule
NVIDIA RTX 3090 (24~GB) & 40~GB & $\sim$40~ms (reference) & 1.0$\times$ \\
...
\section{Mie vs.\ T-Matrix Sensitivity Corrections}\label{app:grain_sensitivity}

Table~\ref{tab:grain_sensitivity} provides the full grain-sensitivity data referenced from \S\ref{sec:uncertainty_budget}.
...
\bottomrule
\end{tabular}
\tablecomments{Positive $\Delta$ = Mie overestimates relative to T-matrix (porosity $P=0.3$). T-matrix: prolate spheroids, $a/b=1.4$--2.0, monomer $a_0=0.1$--1.0\,$\mu$m, $P=0.3$/$0.5$, $a_{\rm eff}=1$\,$\mu$m \citep{mishchenko02}. Corrections valid only for these aggregate parameters.}
\end{table}

\section{Extended Discussion: OOD Detector Independence Assumption}\label{app:independence}

The 0.4\% residual uncaught risk reported in \S\ref{sec:ood} is computed as $P(\text{FN}_{\rm OOD}) \times (1 - P(\text{capture} \mid \text{FN}_{\rm OOD})) = 6.0\% \times 7.0\% \approx 0.4\%$, which assumes statistical independence between the GMM-based OOD detector and the boundary proximity alert.

\textbf{Why independence is defensible as a first-order approximation.} The two mechanisms operate on fundamentally different representations: the OOD detector scores log-likelihood in the GMM-fitted latent space (64-dimensional PCA-whitened encoder features), while the boundary proximity alert checks whether the PINN's output value falls within 10\% of the training range boundary. Because they operate on different layers of the pipeline (latent features vs.\ final parameter estimates), they respond to different properties of the input.

\textbf{Why independence may not hold.} Both mechanisms are functions of the same input $\mathbf{x}$. A complete failure of the encoder to represent a given OOD input could simultaneously produce (a) a high-likelihood latent code (defeating the GMM) and (b) a saturated-but-boundary-proximal estimate (defeating the alert). If the mechanisms are positively correlated---i.e., boundary-proximal estimates are more likely when the GMM also fails---the residual risk could exceed 0.4\%. Conversely, negative correlation would make 0.4\% conservative.

\textbf{Why empirical verification is impossible.} With only 2 uncaught cases ($N=2$), the correlation structure cannot be estimated; a meaningful estimate requires at least 20--30 uncaught cases. Generating additional extreme-OOD synthetic test cases would increase the sample size but would not address the fundamental issue: on real data, the true composition is unknown, so false negatives are unidentifiable.

\textbf{Bottom line:} The 0.4\% figure is an order-of-magnitude estimate ($\sim 10^{-3}$), bounded above by the 6\% OOD false-negative rate. It should be cited as ``approximately 1 in 250 under the independence approximation'' and never as a validated reliability guarantee.
