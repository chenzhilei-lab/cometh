...
\begin{abstract}
We present \textsc{Cometh}, a community, physics-constrained benchmark protocol and open-source software pipeline for cometary analysis at low signal-to-noise ratios (SNR~$\sim$1--5). The protocol comprises: (i) a six-test synthetic stress suite that maps quantitative failure boundaries for both traditional methods and deep learning approaches under deliberately extreme observing conditions; (ii) a reproducible Af$\rho$ aperture photometry pipeline validated on 231 HST/WFC3 and VLT/FORS2 images of 2I/Borisov (201 measurements, 11 epochs, cross-instrument ratio 1.03); and (iii) a reference implementation integrating a multi-scale CNN, a physics-informed neural network (PINN) with hard conservation constraints, and an out-of-distribution (OOD) detector, all with complete architectural disclosure and fixed random seeds for exact reproducibility. \textbf{This is a methodology and benchmark protocol, not a validated measurement tool.} All quantitative performance metrics are synthetic-benchmark-only. The reference PINN saturates at its training boundary for compositions far outside the training range (training range: CO/H$_2$O $\in [0.01, 2.0]$; e.g., CO/H$_2$O~$=5.0$ predicted as $2.08\pm0.15$, a $>10\sigma$ catastrophic error), demonstrating a ``precise-but-wrong'' failure mode that the OOD detector and boundary proximity alert are designed to flag. No \textsc{Cometh} inference has been performed on real 3I/ATLAS data. Validation of the protocol on real interstellar comets awaits $N \geq 5$ multi-modal objects (currently $N=2$). The protocol is designed as a community resource: any future cometary analysis method---traditional or machine-learning-based---can be evaluated against the stress suite and the Af$\rho$ baseline on equal terms. Code, configuration files, and data generation scripts are available at \url{https://anonymous.4open.science/r/cometh-F185}; trained weights will be released upon acceptance. A community validation roadmap is provided in \S\ref{sec:roadmap}.
\end{abstract}
...
The recently discovered 3I/ATLAS (C/2025 N1) has been extensively characterized by the community---over 85 peer-reviewed papers as of mid-2026---spanning nine distinct observational dimensions. Table~\ref{tab:3i_atlas_obs} organizes these by research dimension, observational platform, key numerical constraints, and the corresponding \textsc{Cometh} module that each observation is intended to inform. Critically, the CO$_2$/H$_2$O ratio of $7.6 \pm 0.3$ measured by JWST/NIRSpec \citep{cordiner25} confirms that extreme volatile enrichment occurs in interstellar comets---motivating, in general, the ``Alien Chemistry'' stress test (CO/H$_2$O~$=5.0$, far outside the PINN training range of CO/H$_2$O $\in [0.01, 2.0]$; \S\ref{sec:stress}, Test~5) that probes composition space far beyond the solar-system distribution. \textbf{No \textsc{Cometh} inference has been performed on real 3I/ATLAS data; all ``3I/ATLAS'' results in this paper are synthetic case studies.} The real 3I/ATLAS observations serve as motivation for the extreme-parameter stress tests, not as validation targets.
...
\midrule
NIR spectroscopy & \citet{cordiner25} & JWST/NIRSpec (0.6--5.3\,$\mu$m) & CO$_2$/H$_2$O $= 7.6 \pm 0.3$ & Cross-modal attn.\ (spectral branch) & Extreme CO$_2$ enrichment; motivates general concern about non-solar volatile ratios (cf.\ CO/H$_2$O~$=5.0$ in Stress Test~5) \\
...
\bottomrule
\end{tabular}
\tablecomments{All listed references are published or in press as of mid-2026. The CO$_2$/H$_2$O~$=7.6\pm0.3$ from JWST/NIRSpec is the single most constraining datum for this paper's stress-test design: it confirms that 3I/ATLAS has a composition more extreme than any solar system comet, justifying the CO/H$_2$O~$=5.0$ boundary in Stress Test~5. \textbf{Important:} no \textsc{Cometh} inference has been performed on any real 3I/ATLAS data. The ``Cometh Module'' column indicates intended future application only.}
\end{table*}
...
\bottomrule
\end{tabular}
\tablecomments{All performance figures are from synthetic benchmarks with known ground truth. Speedup values are hardware-dependent: GPU timings on NVIDIA A100, CPU timings on AMD EPYC 7543. The $\sim$100$\times$ wall-clock figure compares GPU-DL vs.\ CPU-Traditional and conflates GPU acceleration ($\sim$30$\times$) with automation ($\sim$3.5$\times$). On equal CPU hardware, pure-compute parity ($\sim$1$\times$). See Table~\ref{tab:timing} for the full three-way decomposition. No claim of real-data accuracy is made beyond the $N=1$ post-hoc consistency check on 2I/Borisov (\S\ref{sec:traditional_comparison}).}
\end{table*}
...
\begin{enumerate}
    ...
    \item \textbf{N=1 post-hoc consistency check, not statistical validation:} A single real object with published multi-modal data can provide a weak necessary condition---if the framework disagrees catastrophically with literature values, it is wrong---but cannot establish accuracy (\S\ref{sec:borisov_check}). Population-level claims require $N \geq 5$ (\S\ref{sec:mmd_gap}).
...
\small
\textbf{Algorithm:} PINN Forward Pass with Hard Physical Constraints \\[3pt]
\textbf{Input:} Image $\mathbf{x} \in \mathbb{R}^{H \times W}$, spectrum $\mathbf{s}$ (optional) \\
\textbf{Output:} $\{Q_d, \text{CO/H}_2\text{O}, \alpha, \theta_{\rm jet}, v_d(r)\}$ with guaranteed physical consistency \\[3pt]
\rule{\linewidth}{0.4pt}
\begin{enumerate}
    \item[\textbf{1.}] \textbf{Encode:} $\mathbf{z} \leftarrow \text{Enc}_\phi(\mathbf{x}, \mathbf{s})$ \hfill $\triangleright$ 256-dim latent features \\
    \hspace{1.2em}(4 residual blocks, GroupNorm, SiLU; Table~\ref{tab:pinn_arch})
    \item[\textbf{2.}] \textbf{Predict unconstrained latents:} \hfill $\triangleright$ MLP head, all $\tilde{\cdot} \in \mathbb{R}$ \\
    \hspace{1.2em}$\tilde{Q}_d, \tilde{v}_d, \tilde{Q}_g, \tilde{\alpha}, \tilde{\theta}_{\rm jet}, \tilde{T} \leftarrow \text{MLP}_\psi(\mathbf{z})$
    \item[\textbf{3.}] \textbf{Hard Mass Conservation (Eq.~\ref{eq:hard_mass}):} \hfill $\triangleright$ zero trainable params \\
    \hspace{1.2em}$Q_d \leftarrow \text{softplus}(\tilde{Q}_d)$ \hfill $\triangleright$ positivity guarantee \\
    \hspace{1.2em}$v_d \leftarrow \text{softplus}(\tilde{v}_d)$ \\
    \hspace{1.2em}$n_d(r) \leftarrow Q_d \,/\, (4\pi r^2 v_d)$ \hfill $\triangleright$ $r^{-2}$ profile by construction
    \item[\textbf{4.}] \textbf{Hard Jet Collimation (Eq.~\ref{eq:hard_angular}):} \hfill $\triangleright$ zero trainable params \\
    \hspace{1.2em}$P_{\rm sub} \leftarrow \text{ClausiusClapeyron}(\tilde{T})$ \hfill $\triangleright$ sublimation pressure \\
    \hspace{1.2em}$x \leftarrow (\dot{m}_g v_g) \,/\, (4\pi r_n^2 P_{\rm sub})$ \\
    \hspace{1.2em}$\theta_{\rm jet} \leftarrow 2\arcsin(\min(1, \sqrt{x}))$ \hfill $\triangleright$ energy-limited cap
    \item[\textbf{5.}] \textbf{Momentum Conservation (Eq.~\ref{eq:dust_gas_ode}):} \hfill $\triangleright$ differentiable ODE \\
    \hspace{1.2em}$v_d(r) \leftarrow \text{RK4}\bigl(\frac{dv_d}{dr} = f(v_d, \rho_g(r), a),\; r \in [r_n, r_{\rm max}]\bigr)$ \\
    \hspace{1.2em}(100 radial steps; IC: $v_d(r_n)=0$; $<1\%$ of inference FLOPs)
    \item[\textbf{6.}] \textbf{Radiative Transfer Integration:} \hfill $\triangleright$ differentiable forward model \\
    \hspace{1.2em}$I_\lambda^{\rm pred} \leftarrow \int n_d(r)\, \sigma_{\rm sca}(\lambda, a)\, \Phi(\theta)\, dr \;+\; \text{gas emission}$
    \item[\textbf{7.}] \textbf{Loss (Eq.~\ref{eq:pinn_loss}):} \hfill $\triangleright$ \textbf{no} $\mathcal{L}_{\rm cons}$ term \\
    \hspace{1.2em}$\mathcal{L} \leftarrow \mathcal{L}_{\rm data}(I_\lambda^{\rm pred}, I_\lambda^{\rm obs}) + \gamma_1 \mathcal{L}_{\rm PDE} + \gamma_2 \mathcal{L}_{\rm BC}$
    \item[\textbf{8.}] \textbf{Backprop:} $\nabla_{\phi,\psi}\,\mathcal{L}$ via autograd through steps 3--6
\end{enumerate}
\rule{\linewidth}{0.4pt}
\textbf{Design invariant:} Any latent values $(\tilde{Q}_d, \tilde{v}_d, \tilde{Q}_g, \tilde{\alpha}, \tilde{\theta}_{\rm jet}, \tilde{T})$ produce fields $(n_d(r), \theta_{\rm jet}, v_d(r))$ that \textit{automatically} satisfy mass, momentum, and jet-collimation conservation---regardless of data quality, optimization state, or SNR. No constraint-violation penalty term is needed because violation is architecturally impossible. The AdaptiveConstraintManager (\texttt{src/hard\_constraints.py}) can dynamically relax step~3 (mass) and step~5 (momentum) from hard to soft constraints when the OOD detector flags non-steady-state conditions (Stress Test~6; \S\ref{sec:ood}).
\end{minipage}
}
\caption{PINN forward pass with hard physical constraints embedded as deterministic, non-trainable transforms. The network predicts unconstrained latent parameters (step~2); the physics layer (steps~3--5, zero trainable parameters) enforces conservation laws by construction. Gradients flow end-to-end via standard autograd. No constraint-violation penalty term appears in the loss (step~7).}\label{alg:hard_constraints}
\end{figure}
...
The OOD detector complements the Bayesian PINN uncertainty: the Bayesian 95\% CI quantifies \textit{aleatoric and epistemic uncertainty within the training distribution}, while the OOD flag indicates whether the input is \textit{within the training distribution at all}. An input can have a narrow 95\% CI (high within-distribution confidence) and still produce a wrong answer if it is OOD---as Stress Test 5 demonstrated. The OOD flag and boundary proximity alert together provide a layered defense: the OOD flag catches 94\% of extreme-OOD cases, and the boundary proximity alert catches 93\% of the remaining 6\% false negatives, leaving an estimated $\sim$0.4\% residual uncaught rate. Users should treat any output with either flag raised as a limit rather than a measurement.
...
\textbf{Construction:} Synthetic cometary spectra with CO/H$_2$O = 5.0 and C$_2$/CN = $-1.0$---values more extreme than the measured 2I/Borisov CO/H$_2$O ($1.30$--$1.55$; \citealt{bodewits20}) and far outside the training range (CO/H$_2$O $\in [0.01, 2.0]$). We emphasize that this test targets CO/H$_2$O, which is distinct from the CO$_2$/H$_2$O ratio measured for 3I/ATLAS (CO$_2$/H$_2$O $= 7.6 \pm 0.3$; \citealt{cordiner25}). While 3I/ATLAS demonstrates that extreme CO$_2$ enrichment can occur in interstellar comets---and while CO$_2$ and CO are chemically distinct species with different formation pathways---the fact that 3I/ATLAS's volatile ratios deviate so dramatically from the solar-system distribution provides general plausibility for the hypothesis that other volatiles (including CO) may also be anomalously enriched in interstellar comets. The CO/H$_2$O ratio of 5.0 tested here is a round-number conservative upper bound: 2I/Borisov's CO/H$_2$O ($\sim$1.5) is already $\sim$30$\times$ the solar-system median \citep{bockelee04}, and CO/H$_2$O~$=5.0$---approximately three times 2I/Borisov's enrichment---provides a clear extrapolation test beyond the training range maximum (2.0) while remaining within the physically plausible regime for carbon-rich cometary material. The dust is modeled as pure amorphous carbon ($m = 2.2 + 0.5i$) rather than astronomical silicate, producing radically different scattering properties.
...
A representative visual comparison across all six stress test scenarios (available in the public repository as \texttt{stress\_test\_examples/}) is described schematically here. Each panel shows: (left) the raw synthetic input image under the stress condition; (center) the output of the best traditional method (non-local means for Tests 1--3, 5--6; Haser+$Af\rho$ for Test 4); (right) the output of the proposed methodology. The figure is designed to serve as a qualitative ``at a glance'' summary of the Fail Gallery (\S\ref{sec:fail_gallery}), complementing the quantitative metrics in Tables~\ref{tab:detection_metrics}--\ref{tab:pin_perf}. Key visual signatures: (a) Test 1 (galactic plane)---traditional method leaves stellar residuals mimicking cometary fragments; the proposed methodology suppresses them while preserving the true coma. (b) Test 3 (SNR~$=0.75$)---traditional method produces a noise field with no recoverable structure; the proposed methodology recovers the coma outline at reduced fidelity. (c) Test 5 (alien chemistry)---both approaches produce visually similar outputs, but the OOD detector flags the input while the traditional method silently produces a plausible-looking but incorrect result. The complete test image set is included in the public repository.
...
\bottomrule
\end{tabular}
\tablecomments{1000-image synthetic test set at SNR~$=2$. All rows are detection CNN ablations evaluated with image-quality metrics (PSNR/SSIM/LPIPS). The PINN hard-constraint ablation is reported separately in \S\ref{sec:pinn} and Table~\ref{tab:pin_perf}, using physical parameter metrics (MAPE on $Q_d$) appropriate for the inversion module. All $\Delta$PSNR $\geq 0.7$~dB are significant at $p < 0.01$ (paired $t$-test, Holm--Bonferroni correction for 7 comparisons).}
\end{table*}
...
OOD detector & 94\% OOD detection (includes trivially-OOD cases far from boundary; 6\% false-negative on boundary-proximal cases) & Stress Test 5 & \S\ref{sec:ood} \\
Cross-modal attention (fusion) & SNR gain $+0.3$~dB over late fusion & SNR~$=2$, multimodal inputs & \S\ref{sec:fusion} \\
Attention consistency loss ($\mathcal{L}_{\rm attn}$) & FPR $3.0\% \to 2.1\%$ on synthetic injection test & SNR~$=2$, 1000 images & \S\ref{sec:xai} \\
\bottomrule
\end{tabular}
\tablecomments{All metrics from experiments reported in the cited sections. The cross-modal attention (fusion) row compares uncertainty-weighted fusion with attention against a late-fusion baseline (concatenation + linear projection). The attention consistency loss row is a training-time regularizer that penalizes Grad-CAM deviations from injection masks (\S\ref{sec:xai}), separately benchmarked from the fusion mechanism. \textbf{Caveat:} These ablation results are drawn from synthetic data with known ground truth; their transferability to real interstellar comet observations has not been verified.}
\end{table}
...
\subsection{Low-CO/H$_2$O Boundary Test: Hale-Bopp as a Solar System Distribution Probe}\label{sec:proxy}
...
\begin{table}[htbp]
\centering
\caption{Framework vs.\ Published Traditional Analyses on 2I/Borisov (Literature Values, Not Same-Data Re-analysis; $N=1$)}\label{tab:traditional_comparison}
\begin{tabular}{lcccc}
\toprule
...
\subsubsection{Same-Data Direct Comparison: $Af\rho$ on Identical 2I/Borisov Images (Type~B Evidence)}\label{sec:same_data}
...
\begin{enumerate}
    ...
    \item \textbf{Synthetic-to-real MMD gap:} As discussed in \S\ref{sec:mmd_gap}, the domain adaptation validation is restricted to MMD reduction between two \textit{synthetic} distributions. Computing the MMD between synthetic and real interstellar comet features requires a multi-object sample that does not yet exist. \textbf{Partial workaround:} A Physical Sensitivity Envelope (PSE)---systematically perturbing each physical assumption (spherical grains, optically thin coma, steady-state Haser outflow) within the synthetic data generator and measuring the resulting change in PINN output---provides an alternative uncertainty quantification that does not require real interstellar comet samples. The PSE yields an operational envelope: ``if real comets deviate from assumptions by $X$, then COMETH outputs change by $Y$.'' This approach, partially implemented in \S\ref{sec:uncertainty_budget}, shifts the validation question from ``does the synthetic distribution match the real one?'' (unanswerable with $N=1$) to ``how wrong would COMETH be if our assumptions are wrong by a specified amount?'' (answerable with synthetic perturbation experiments). As a concrete illustration: \S\ref{sec:uncertainty_budget} already reports that replacing spherical Mie grains with moderately porous T-matrix aggregates (porosity $P=0.3$) changes $Q_d$ by $+8.2\%$; extending this to $P=0.5$ yields $+18.5\%$. A complete PSE would tabulate analogous sensitivity coefficients for optical depth ($\tau$), Haser scale-length uncertainty, and non-steady-state amplitude, producing a lookup table that future users can consult to estimate systematic error without requiring real comet ground truth. A systematic PSE covering all four major assumptions is deferred to future work.
...
\begin{enumerate}
    \item \textbf{Not a measurement paper:} No physical parameters of any real comet are measured. All ``outputs'' on 2I/Borisov are post-hoc consistency checks against published literature values.
...
\textsc{Cometh} is a community benchmark protocol and reference implementation, not a validated measurement tool. All quantitative performance metrics are synthetic-benchmark-only. The 2I/Borisov pipeline execution test confirms that the software runs end-to-end on real data; it does not validate parameter accuracy. The OOD detector demonstrates the \textit{principle} of distribution-shift detection on synthetic data, but its false-negative rate on real interstellar comets is unmeasurable with $N=1$. The six-test stress suite and open-source Af$\rho$ pipeline are available today as community benchmarks; the PINN-based parameter estimation requires $N \geq 5$ real interstellar comets for validation. The complete source code, stress test suite, and Docker environment are available at \url{https://anonymous.4open.science/r/cometh-F185}; trained model weights will be released upon acceptance (training carbon footprint order $\sim$50~kg~CO$_2$).
