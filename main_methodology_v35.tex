...
\textbf{Hardware requirements and consumer-hardware benchmarking.} Table~\ref{tab:consumer_hw} provides estimated end-to-end inference times across common hardware tiers, scaled from the reference A100 measurement using published GPU throughput ratios and vendor specifications. These are \textbf{estimated} values (not independently measured on every listed device); the scaling methodology and caveats are described below the table.

\begin{table}[htbp]
\centering
\caption{Estimated End-to-End Inference Time per Observation on Common Hardware}\label{tab:consumer_hw}
\begin{tabular}{lccc}
\toprule
...
CPU: AMD EPYC 7543 (32-core)$^\ddagger$ & --- & $\sim$1.2~s & $\sim$30$\times$ \\
CPU: Laptop (8-core, e.g., Apple M2 CPU) & --- & $\sim$2--4~s & $\sim$50--100$\times$ \\
\bottomrule
\end{tabular}
\tablecomments{$^\dagger$Per-observation end-to-end inference (detection CNN + PINN inversion + OOD detector). CPU timings use PyTorch CPU backend. GPU timings assume CUDA~12.1 or MPS (Apple Silicon). $^\ddagger$CPU reference: same AMD EPYC 7543 node used for the traditional-pipeline timing in Table~\ref{tab:timing}. \textbf{Scaling methodology:} Estimates for non-A100 hardware are derived by scaling the measured A100 latency (40~ms, $\sim$6.8~GFLOPs) by the relative FP32 throughput ratios (RTX 4090 $\approx$ 0.55$\times$ A100 FP32; RTX 3090 $\approx$ 0.45$\times$; RTX 3060 $\approx$ 0.22$\times$; Apple M2 GPU $\approx$ 0.12$\times$; CPU $\approx$ 0.03$\times$), then rounding to the nearest 10~ms to avoid false precision. These are first-order estimates that do not account for memory bandwidth bottlenecks, PyTorch kernel optimization differences, or MPS backend maturity. RTX 3060 (12~GB) can run inference but \textbf{cannot} train (requires $\geq$12~GB, and 12~GB is marginal); pre-trained weights from Zenodo must be used. Apple Silicon (MPS) inference is supported via PyTorch~2.1 MPS backend but has not been benchmarked in this work; estimate assumes MPS FP32 throughput at 50\% of peak. \textbf{All consumer-hardware timings should be verified on the target device before operational use.}}
\end{table}

\textbf{Training:} Requires a GPU with $\geq$12~GB VRAM (NVIDIA A100, V100, RTX 3090, or RTX 4090). RTX 3060 (12~GB) can train with reduced batch size but is not recommended due to marginal VRAM. Apple Silicon GPUs are \textbf{not} recommended for training (MPS backend limitations with custom CUDA kernels used in the DIRTY forward model).

\textbf{Inference:} Runs on any GPU with $\geq$4~GB VRAM (including RTX 3060 and most consumer GPUs), or on CPU with $\sim$30--60$\times$ slowdown relative to A100. The memory footprint is $\sim$3.2~GB (GPU) or $\sim$4~GB system RAM (CPU), making CPU-only inference feasible on any modern laptop ($\geq$8~GB RAM). CPU inference produces identical numerical outputs to GPU inference (same floating-point operations, different execution order, $\lesssim 10^{-5}$ relative difference).

\textbf{Operational guidance for small research groups:} (1) \textbf{No GPU:} Download pre-trained weights from Zenodo, run CPU inference (2--4~s per observation on a laptop; $\sim$5~min for a typical 100-image observing block). The Af$\rho$ pipeline and stress test suite are CPU-only and require no GPU. (2) \textbf{Consumer GPU (RTX 3060/4060, 12~GB):} Run GPU inference (150--200~ms/obs). Training is possible with reduced batch size but slow ($\sim$4--8$\times$ A100 training time, $\sim$1500--3000~GPU-hours for full training). Use pre-trained weights unless hyperparameter modifications are required. (3) \textbf{Apple Silicon Mac (M1/M2/M3):} Run MPS-accelerated inference (300--500~ms/obs) using the PyTorch MPS backend. Training is not supported due to DIRTY's CUDA dependency. (4) \textbf{Cloud GPU rental (RTX 3090/4090):} At market rates ($\sim$0.30--0.50 USD/hr for RTX 3090, $\sim$0.50--0.80 USD/hr for RTX 4090), the $\sim$350~GPU-hour training cost translates to $\sim$100--280 USD on consumer cloud instances---an order of magnitude cheaper than A100 cloud instances ($\sim$2.00/hr, $\sim$700 USD). All reported timings use PyTorch~2.1.0 with CUDA~12.1, NVIDIA A100 (40~GB), and AMD EPYC 7543 CPU.
