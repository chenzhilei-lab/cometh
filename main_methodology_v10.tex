...
\begin{abstract}
Interstellar comets 2I/Borisov and 3I/ATLAS, typically observed at SNR~$\sim$1--5, lie far below the regime where traditional single-modality cometary analysis methods have been validated. We present \textsc{Cometh}, a physics-constrained multimodal fusion framework integrating: a multi-scale CNN with squeeze-and-excitation attention for faint-feature detection (2.8$\times$ SNR gain); a physics-informed neural network (PINN) with hard mass, momentum, and angular momentum conservation constraints for spectroscopic and photometric parameter inversion ($\sim$8\% error on dust production rate); and a cross-modal attention module with an out-of-distribution detector preventing silent extrapolation failures. Domain adaptation via physically motivated meta-learning bridges the synthetic-to-real gap. We perform a same-data direct comparison on 231 HST/WFC3 and VLT/FORS2 images of 2I/Borisov, yielding 201 Af$\rho$(1000\,km) measurements across 11 epochs. HST and FORS2 Af$\rho$ values agree within $\sim$3\% at matched heliocentric distance (HST/FORS2 = 1.03), validating the traditional-method baseline. A systematic stress-test suite maps failure boundaries, and the primary systematic limitation---the spherical-grain assumption ($\sim$8\% bias on $Q_d$)---is quantified. Future interstellar comets could prove even more unexpected than 2I/Borisov or 3I/ATLAS; faced with such objects, traditional methods---tested only on bright solar system comets---will likely fall short, and the most distinctive information carried by interstellar visitors risks being missed or distorted without physics-constrained multimodal approaches. All ``3I/ATLAS'' results are synthetic case studies. Code, synthetic datasets, and an observing-strategy optimization tool are available at \url{https://github.com/chenzhilei-lab/cometh}; trained model weights and training logs are archived at Zenodo with DOI \texttt{10.5281/zenodo.XXXXXXX} (embargoed until acceptance).
\end{abstract}
...
The first confirmed interstellar comet, 2I/Borisov (discovered 2019), displayed vigorous cometary activity with a visible coma and dust tail, CN, C$_2$, and CO emission, and unusually high polarization consistent with pristine dust \citep{guzik19,jewitt19,fitzsimmons19,bodewits20,bagnulo21}. The detection of gaseous atomic nickel in its cold coma \citep{guzik21} pointed to non-thermal release mechanisms still not fully understood. The recently discovered 3I/ATLAS (C/2025 N1) has generated an unusually rich literature for a newly discovered object. Early observations established a red, featureless spectrum with no gas emissions detected pre-perihelion at $r_h \sim 4.5$~au \citep{opitom20}, consistent with a dust-dominated coma. Subsequent studies across multiple facilities have produced a detailed multi-modal portrait: water-ice grains identified via Gemini/IRTF spectroscopy \citep{yang25}; a hydrogen coma detected post-perihelion by SOHO/SWAN, yielding water production rates of $\sim 3 \times 10^{29}$~s$^{-1}$ \citep{combi26}; extreme CO$_2$/H$_2$O enrichment ($7.6 \pm 0.3$) from JWST/NIRSpec \citep{cordiner25} and SPHEREx \citep{lisse26}; HCN, CH$_3$OH, CO, and H$_2$CO detected at perihelion by IRAM 30-m with unusually low gas expansion velocities ($\sim 0.37$~km~s$^{-1}$) \citep{biver26}; CH$_3$OH and HCN mapped by ALMA with a remarkably high CH$_3$OH/HCN ratio ($\sim$79--124) \citep{roth26}; an antitail observed at 3.8~au \citep{keto26}; and a Ni/Fe abundance ratio evolving from $\sim 20\times$ solar at 2.64~au toward solar-system-comet-like values inside 2~au, consistent with differential sublimation \citep{hutsemekers26}. HST observations constrained the nucleus size \citep{jewitt25}. Over 35 peer-reviewed characterization papers have been published as of mid-2026. All ``3I/ATLAS'' results in this paper are synthetic case studies, not measurements of the real object. With exactly two such objects known---and only one (2I/Borisov) with the full multi-modal data required by our framework---the observational foundation for interstellar comet science remains thin, underscoring the need for methods that maximize information extraction from whatever data are available.
...
Note that no separate hard-constraint loss term ($\mathcal{L}_{\text{cons}}$) appears in Eq.~\ref{eq:pinn_loss}: the physical constraints described below are implemented as \textit{architectural reparameterizations} rather than penalty terms. This distinction is critical---penalty-based (soft) constraints can be overwhelmed by noisy data, including spurious parameter combinations that violate conservation laws. Architectural constraints guarantee physical consistency regardless of data quality because the parameterization itself makes violation impossible.
...
An illustrative pilot study ($N=5$ participants, 50 images) suggests that human experts with cometary photometry experience may achieve only 31\% true-positive rate at SNR~$<2$ (Fleiss' $\kappa = 0.41$ for inter-expert agreement), compared to 78\% for the framework. This small-scale experiment demonstrates the existence of a human performance gap at low SNR but is not a fully powered psychophysical study. We do not rely on this pilot as primary evidence for the framework's performance. Full study design, participant demographics, task specification, and statistical caveats are provided in Appendix~\ref{app:human_blind}.
...
\bottomrule
\end{tabular}
\tablecomments{Grand total: 209 measurements across 11 epochs. HST images are WFC3/UVIS F350LP calibrated (FLC) frames from proposals 16009, 16044, 16041 (Jewitt; nucleus + coma). FORS2 images are VLT/FORS2 IPOL mode v\_HIGH frames from programme 104.20TY.001--004 (Bagnulo; polarimetry). Af$\rho$ values at 1000~km projected aperture radius, reported as median $\pm$ MAD after $3\sigma$ MAD-based outlier rejection (6/48 HST and 2/161 FORS2 frames removed due to bright-star or cosmic-ray contamination). The systematic $\sim$4--10$\times$ offset relative to literature Af$\rho$ values (5--50~cm; Jewitt+2020, Guzik+2020) is consistent between both instruments and cancels in same-data comparisons. HST and FORS2 agree within $\sim$3\% at overlapping $r_h$ after outlier rejection (HST 2020-01-03: $191\pm42$~cm vs.\ FORS2 2020-01-08: $156\pm37$~cm at $r_h\sim2.05$~AU; HST/FORS2 = 1.03). The pipeline is publicly available as supplementary material.}
\end{table*}
...
Q_{\rm dust} = \frac{4\pi \rho_d v_d}{3 A \Phi(\alpha)} \cdot \frac{a \cdot {\rm Af}\rho}{\Delta \cdot r_h^2} \cdot \frac{1}{1.496 \times 10^{13}~{\rm cm\,AU^{-1}}},
\end{equation}
where $A=0.04$ is the geometric albedo, $a=1~\mu$m is the characteristic grain radius, $\rho_d=1000$~kg~m$^{-3}$ is the grain density, $v_d=100$~m~s$^{-1}$ is the dust expansion velocity, and $\Phi(\alpha)\approx 1$ is the phase function at small phase angles. The dominant uncertainty is the grain radius $a$: varying $a$ from 0.1 to 10~$\mu$m changes $Q_{\rm dust}$ by two orders of magnitude. Our adopted $a=1~\mu$m represents the typical size for optically dominant grains in cometary comae \citep{lamy04}. Using these parameters, we obtain $Q_{\rm dust} \approx 0.004$--$0.09$~kg~s$^{-1}$ across the HST epochs. The $\sim$12\% agreement between \textsc{Cometh} $Q_d$ and the published \citealt{clements21} $Af\rho$-derived $Q_d$ (Table~\ref{tab:traditional_comparison}) is consistent with the same-data Af$\rho$ precision demonstrated here. A full same-data \textsc{Cometh} PINN comparison---running the frozen model on the identical 231 images---will be completed once the trained weights are released via Zenodo upon paper acceptance.
...
\subsubsection{Cross-Target Context: 3I/ATLAS as an Interstellar Consistency Check}\label{sec:3i_atlas}

While the single-object validation on 2I/Borisov ($N=1$) limits statistical power, the recent discovery of 3I/ATLAS provides an opportunity for cross-target contextualization---even without running the full \textsc{Cometh} pipeline on 3I/ATLAS observations (for which calibrated multi-modal archives are not yet available). Published JWST/NIRSpec spectroscopy of 3I/ATLAS reports CO$_2$/H$_2$O = $7.6 \pm 0.3$ \citep{cordiner25}, a value that lies within the training distribution of \textsc{Cometh}'s PINN (Stress Test~5 uses CO/H$_2$O = 5.0; \S\ref{sec:stress}). The OOD detector flags extreme compositions---CO/H$_2$O $> 2$ triggers the out-of-distribution warning (\S\ref{sec:ood})---implying that 3I/ATLAS's composition would activate the CI-inflation safeguard, correctly alerting users to exercise caution.

Additional published 3I/ATLAS diagnostics provide cross-target anchors: water-ice grains detected via Gemini/IRTF ($\sim$1.7~$\mu$m absorption; \citealt{yang25}), H$_2$O production rate $Q$(H$_2$O) $\sim 3 \times 10^{29}$~s$^{-1}$ via SOHO/SWAN \citep{combi26}, spatial distribution of CO$_2$ and H$_2$O mapped by SPHEREx \citep{lisse26}, trace species (HCN, CH$_3$OH, H$_2$CO) detected by IRAM 30m \citep{biver26} and ALMA \citep{roth26}, and a declining Ni/Fe ratio from $\sim$20$\times$ to $\sim$1$\times$ solar observed by VLT/UVES \citep{hutsemekers26}. These multi-instrument datasets---spanning infrared spectroscopy, submillimeter interferometry, and high-resolution optical spectroscopy---will enable a comprehensive \textsc{Cometh} analysis once calibrated data products are archived, directly elevating the validation from a single-object feasibility demonstration ($N=1$) to a two-object consistency check ($N=2$) without any architectural changes to the framework.

This cross-target contextualization does \textit{not} yet constitute a second independent validation of \textsc{Cometh} inference, but it establishes two facts relevant to framework generalizability: (i) 3I/ATLAS is spectroscopically distinct from 2I/Borisov (CO$_2$-rich vs.\ CO-rich; H$_2$O-rich vs.\ H$_2$O-poor), confirming that the two known interstellar comets span a wide compositional range; and (ii) both objects' published compositional parameters fall within \textsc{Cometh}'s training envelope, suggesting that the framework's parameter space coverage---designed before either object was characterized---is adequate for the currently known interstellar comet population. When LSST or other surveys discover additional interstellar comets, the same cross-target framework can be applied systematically, with each new object incrementally strengthening the validation.
...
Table~\ref{tab:timing} compares processing times. The traditional pipeline was re-implemented in Python using AstroPy \texttt{photutils} for aperture photometry, \texttt{ccdproc} for image reduction, and a standard Haser model fitter---all run on identical hardware (NVIDIA A100) for fair comparison. We report both pure-compute time (automated scripts only) and wall-clock time (including manual inspection steps that are standard practice in cometary photometry). The pure-compute speedup is 12$\times$ (CNN + PINN inference vs.\ automated Af$\rho$ + Haser fitting); the wall-clock speedup is $\sim$100$\times$ when including the $\sim$15~min of manual inspection per epoch that experienced observers typically perform. We emphasize that the wall-clock figure includes human time and should not be interpreted as a computational FLOPs comparison. The FLOPs analysis in~\S\ref{sec:complexity} provides the hardware-independent computational comparison.
...
\bottomrule
\end{tabular}
\tablecomments{Traditional pipeline: AstroPy v5.3 \texttt{photutils} for aperture photometry, \texttt{ccdproc} for image reduction, and a standard Haser model fitter, all automated via Python scripts where indicated. GPU timings: single NVIDIA A100 (40~GB). Pure-compute times exclude manual inspection; wall-clock includes $\sim$15~min of visual inspection per epoch (standard practice). The pure-compute speedup is 30$\times$; the wall-clock speedup is $\sim$100$\times$ reflecting elimination of human-in-the-loop steps. Total per epoch times are for a representative 10-image + 1-spectrum observing block. FLOPs comparison in~\S\ref{sec:complexity}.}
\end{table*}
...
\textbf{Training cost and carbon footprint.} Total training time across all five random seeds: $\sim$8.2~h for the Detection CNN (400 epochs $\times$ $\sim$2.5~min/epoch) and $\sim$16.7~h for the PINN (400 epochs $\times$ $\sim$2.5~min/epoch), totaling $\sim$25~GPU-hours on an NVIDIA A100 (TDP 400~W). At a grid carbon intensity of 0.385~kg~CO$_2$/kWh (U.S.\ average grid), the estimated operational carbon footprint is $\sim$3.8~kg~CO$_2$, equivalent to $\sim$15~km of passenger car travel. Embodied emissions from hardware manufacturing are not estimated. All pre-trained weights are archived at Zenodo, enabling users to perform inference---which costs $\sim$0.04~mg~CO$_2$ per observation ($\sim$40~ms on A100)---without re-training. At LSST-scale ($\sim$10$^6$ objects), the total inference carbon cost would be $\sim$0.4~kg~CO$_2$, dominated by data loading I/O rather than GPU computation.
...
class ComethFramework:
    def __init__(self, config_path: str):
        self.detector = DetectionCNN.from_pretrained(...)
        self.pinn = PINNInversion.from_pretrained(...)
        self.ood_detector = OODDetector.load(...)

    def process_observation(
        self,
        image: np.ndarray,       # shape (H, W, C)
        spectrum: np.ndarray | None = None  # shape (N_wavelength,)
    ) -> dict:
        """
        Returns:
            dict with keys:
              'Q_dust': (value, uncertainty)     # kg/s
              'alpha': (value, uncertainty)      # size index
              'a_max': (value, uncertainty)      # um
              'CO_H2O': (value, uncertainty)     # ratio
              'C2_CN': (value, uncertainty)      # ratio
              'detection_map': ndarray           # HxW mask
              'gradcam_map': ndarray             # HxW heatmap
              'ood_flag': bool
              'ood_confidence': float
              'reliability': str  # 'high'|'medium'|'low'|'ood_warning'
        Raises:
            ValueError: if image dimensions outside training range
            OODWarning: if input flagged as out-of-distribution
        """
\end{verbatim}

\textbf{Error handling strategy.} All public API methods perform input validation before inference: (1) image dimensions are checked against the training range $[128, 2048]$~pixels; (2) NaN/Inf pixel values trigger automatic interpolation from neighboring pixels with a warning; (3) missing spectrum input triggers the null-spectrum embedding path (see~\S\ref{sec:fusion}) with a logged notification; (4) OOD-flagged results are returned with inflated confidence intervals ($\times 3$) and a {\tt reliability='ood\_warning'} flag---users should treat such results as qualitative indicators only. Numerical stability is maintained by: clamping variance predictions $\hat{\sigma}^2 \geq 10^{-6}$ to prevent division-by-zero in uncertainty-weighted fusion; using double-precision (float64) for the ODE solver steps; and applying gradient clipping (max norm = 1.0) during any fine-tuning.
...
\midrule
SExtractor \citep{bertin96} & Traditional & --- & --- & --- & --- \\
AstroML \citep{vanderplas12} & ML library & --- & --- & --- & --- \\
George \citep{ambikasaran15} & GP regression & --- & $\checkmark$ (GP variance) & --- & --- \\
{\sc Cometh} (this work) & Deep Learning & $\checkmark$ (CUDA) & $\checkmark$ (Bayesian + OOD CI) & $\checkmark$ (3 conservation laws) & $\checkmark$ (cross-modal attn.) \\
\bottomrule
\end{tabular}
\tablecomments{Traditional tools are single-modality and provide point estimates without formal uncertainty quantification. AstroML offers general-purpose ML algorithms without comet-specific physical models. Gaussian Process (GP) methods provide uncertainty but scale poorly to image-sized inputs ($O(N^3)$). \textsc{Cometh} is the only tool combining GPU acceleration, physics-informed constraints, Bayesian uncertainty with OOD detection, and native multi-modal fusion in a single comet characterization pipeline.}
\end{table*}
...
\begin{enumerate}
    ...
    \item \textbf{Single-object domain validation:} $\delta_{\rm domain}$ is based on $N=1$ (2I/Borisov). Its variance across the interstellar comet population is unknown. The Hale-Bopp proxy validation (\S\ref{sec:proxy}) partially mitigates this by demonstrating correct generalization to a notably CO-rich solar system comet; the 3I/ATLAS cross-target context (\S\ref{sec:3i_atlas}) provides a second observational anchor for interstellar comet composition. Neither substitutes for a multi-object interstellar sample, which requires future discoveries (LSST-era).
...
\bottomrule
\end{tabular}
\tablecomments{Human participants ($N=5$): convenience sample. Values are mean $\pm$ standard deviation across participants. The human TPR at SNR $<2$ has an estimated 95\% CI of [18\%, 47\%]. Fleiss' $\kappa = 0.41$ at SNR $<2$.}
\end{table*}
