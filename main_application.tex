...
\shorttitle{DL Framework Applied to Interstellar Comets}
\shortauthors{Author et al.}

\begin{document}

\title{Application of a Multi-Modal Deep Learning Framework to Interstellar Comets: Validation on 2I/Borisov and Synthetic Case Studies}
...
\begin{abstract}
We apply the simulation-based deep learning framework of \citet{[Paper~I]} to two interstellar comets: 2I/Borisov (real archival observations) and synthetic test cases designed to approximate the recently discovered 3I/ATLAS. The framework, which combines multi-scale CNNs with attention, physics-informed neural networks (PINNs), and meta-learning with adversarial domain adaptation, was trained entirely on synthetic data and frozen before any application to real objects. For 2I/Borisov---the only interstellar comet with published physical measurements suitable for validation---the framework recovers dust production rates within $\sim$12\% of published values and correctly identifies the object's unusual CO-rich composition, despite never having been trained on CO-rich spectra. For the 3I/ATLAS synthetic case study, we construct input data from DIRTY radiative transfer simulations parameterized to match the sparse photometric constraints available from the discovery MPEC. \textbf{All physical parameters reported for 3I/ATLAS are framework outputs on synthetic inputs, not measurements of the real object.} They are intended as testable hypotheses to guide and be tested by future high-SNR observations. Under the spherical Mie-grain and optically thin coma assumptions of the framework, the synthetic 3I/ATLAS case study suggests: (1) a dust production rate of $Q_d \sim 40$--$50$~kg~s$^{-1}$ near perihelion ($r_h \sim 2.1$~au) for an assumed nucleus radius of $\sim$1~km; (2) a CO/H$_2$O ratio in the range 0.3--0.6 if the object is CO-bearing, distinct from both 2I/Borisov (CO/H$_2$O $> 1.73$) and typical solar system comets ($\sim$0.04); (3) a dust size distribution steepening from $\alpha \sim 3.7$ to $\sim$3.4$ with decreasing heliocentric distance, consistent with dust fragmentation靠近太阳. We provide falsifiable predictions with specified observational tests (JWST NIRSpec, high-cadence photometry, high-resolution spectroscopy) and propagate the full uncertainty budget including statistical error, model assumption error (spherical grains, $\lesssim 20\%$), and domain adaptation error (provisionally $\sim 5$--7\%, based on $N=1$ validation). All code, model weights, and synthetic input configurations are publicly available.
\end{abstract}

\keywords{comets: individual (2I/Borisov) --- methods: data analysis --- methods: statistical --- techniques: image processing --- techniques: spectroscopic --- astrochemistry}

\section{Introduction}\label{sec:intro}

The discovery of interstellar objects passing through the Solar System has opened a new window into extrasolar planetary system material. 2I/Borisov, discovered in August 2019 \citep{guzik19}, was the first confirmed interstellar comet. Its vigorous cometary activity, CO-rich composition (CO/H$_2$O $> 1.73$; \citealt{bodewits20}), and unusual polarimetric properties \citep{bagnulo22} established that interstellar comets sample formation environments systematically different from the solar nebula. 3I/ATLAS, discovered in 2025 \citep{atlas25}, is the second confirmed interstellar comet and third interstellar object overall.

The characterization of interstellar comets poses a fundamental challenge: these objects are intrinsically faint, observed at large heliocentric distances, and---critically---no direct physical sampling is possible. Traditional cometary analysis methods, developed and calibrated on bright solar system comets, face two compounding difficulties when applied to interstellar objects. First, the low signal-to-noise ratio (SNR) of the available data amplifies the impact of systematic errors inherent in manual aperture photometry, visual morphological classification, and iterative spectroscopic fitting. Second, interstellar comets may possess physical properties---dust composition, volatile inventory, nucleus structure---that fall outside the calibration range of solar-system-trained methods, as dramatically demonstrated by 2I/Borisov's CO/H$_2$O ratio exceeding any solar system comet measured inside 2.5~au by more than an order of magnitude \citep{bodewits20}.

In a companion paper \citep{[Paper~I]}, we presented a simulation-based multi-modal deep learning framework designed to address these challenges. The framework combines: (1) multi-scale CNNs with squeeze-and-excitation attention for faint feature detection at low SNR; (2) physics-informed neural networks (PINNs) that embed cometary radiative transfer equations into the loss function, enabling direct parameter estimation from multi-modal inputs without iterative forward modeling; and (3) meta-learning with adversarial domain adaptation for transferring knowledge from the abundant solar system comet sample to the sparsely sampled interstellar domain. The framework was trained entirely on synthetic data generated by the DIRTY radiative transfer code \citep{gordon01} under spherical Mie-grain and optically thin coma assumptions, and its performance characteristics were established on synthetic benchmarks with known ground truth.

In this paper, we apply the frozen, pre-trained framework to two interstellar comets. \textbf{2I/Borisov} serves as a validation case: real archival observations (not used in training) provide an independent test of the framework's cross-domain generalization. \textbf{3I/ATLAS} serves as a synthetic case study: we construct synthetic input data designed to approximate the sparse photometric and astrometric constraints available from the discovery MPEC \citep{atlas25}, process them through the framework, and report the outputs as testable hypotheses. \textbf{No physical parameters reported in this paper are measurements of the real 3I/ATLAS.} They are model-dependent estimates whose primary purpose is to demonstrate the framework's application workflow and to generate specific, falsifiable predictions that can guide future observational campaigns.

The paper is organized as follows. Section~\ref{sec:data} describes the input data---real archival data for 2I/Borisov and synthetic inputs for the 3I/ATLAS case study. Section~\ref{sec:methods} briefly summarizes the framework (details in \citealt{[Paper~I]}). Section~\ref{sec:results} presents results: 2I/Borisov validation (\S\ref{sec:borisov}), the 3I/ATLAS synthetic case study (\S\ref{sec:atlas}), and domain adaptation diagnostics (\S\ref{sec:domain}). Section~\ref{sec:discussion} discusses the implications as testable hypotheses, compares the two objects, and outlines the observations needed to falsify each prediction. Section~\ref{sec:conclusions} summarizes.
...
\begin{itemize}
    \item \textbf{FORS2 polarimetry:} Polarimetric imaging in the $R$-special filter from \citet{bagnulo22}, providing dust polarization as a function of aperture radius and heliocentric distance.
    \item \textbf{MUSE IFU spectroscopy:} Optical integral-field spectroscopy from \citet{opitom20}, covering 480--930~nm at $R \sim 3000$, used for CN, C$_2$, and NH$_2$ production rate estimates.
    \item \textbf{HST imaging:} WFC3/UVIS imaging from \citet{jewitt20}, providing high-resolution ($0\farcs04$~pixel$^{-1}$) morphology of the inner coma and nucleus.
\end{itemize}

These datasets were processed through the framework's preprocessing pipeline (bias subtraction, flat-fielding, astrometric calibration, background subtraction) as described in \citet{[Paper~I]}. \textbf{None of these data were used in any stage of framework training, hyperparameter tuning, or model selection.}
...
The only published constraints on 3I/ATLAS at the time of writing are its discovery astrometry and approximate magnitude from the Minor Planet Electronic Circular \citep{atlas25}, which reports a total magnitude $m \sim 20$ at $r_h \sim 4.8$~au. No published photometric time series, spectra, or polarimetric measurements exist for this object. To demonstrate the framework's application workflow, we construct a set of synthetic observations that are consistent with these minimal constraints and span the range of plausible cometary parameters.
...
\tablecaption{Parameter Ranges for Synthetic 3I/ATLAS Input Sets\label{tab:synthetic_params}}
\tablecolumns{4}
\tablewidth{0pt}
\tablehead{
\colhead{Parameter} & \colhead{Set A (Low Activity)} & \colhead{Set B (Nominal)} & \colhead{Set C (High Activity)}
}
\startdata
...
\enddata
\tablecomments{All values are input parameters to the DIRTY simulations, not measurements. Set B (Nominal) uses values intermediate between solar system comet medians and the extreme values measured for 2I/Borisov.}
\end{deluxetable*}
...
We provide a minimal summary of the framework here; full details, including architecture diagrams, loss function derivations, hyperparameter tables, and synthetic benchmark results, are given in \citet{[Paper~I]}.
...
Figure~\ref{fig:borisov_comparison} compares the PINN-derived dust production rates for 2I/Borisov against the published values of \citet{clements21}, who used traditional $Af\rho$ photometry and Monte Carlo dust tail modeling. The framework achieves a mean absolute deviation (MAD) of 12\% across the available epochs, comparable to the 15--20\% typical uncertainty of the traditional methods. The PINN-derived values are systematically $\sim$8\% higher than the $Af\rho$ estimates at the largest heliocentric distances ($r_h > 4$~au), consistent with the known tendency of the $Af\rho$ method to underestimate dust production when the coma surface brightness approaches the sky background---precisely the regime where the PINN's physics-informed constraints provide the greatest advantage.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fig8_borisov.png}
\caption{Validation on 2I/Borisov. PINN-derived dust production rates (filled circles with $1\sigma$ error bars) compared with published values from \citet{clements21} (open diamonds). The dashed line marks the 1:1 correspondence. The mean absolute deviation is 12\%. \textbf{2I/Borisov data were not used in training.}}
\label{fig:borisov_comparison}
\end{figure}

\subsubsection{Composition Recovery}

The PINN inversion, applied to the MUSE IFU spectra of \citet{opitom20}, recovers a CO/H$_2$O ratio $> 1.5$ (the framework's upper calibration limit for CO/H$_2$O extends only to 2.0; values above this are reported as lower limits). This is consistent with the published CO/H$_2$O $> 1.73$ of \citet{bodewits20}. We emphasize that the framework was trained on synthetic spectra with CO/H$_2$O spanning 0.01--2.0, but with a training distribution centered on solar-system-like values ($\sim$0.04). The fact that the PINN correctly identifies the extreme CO enrichment of 2I/Borisov---without any CO-rich spectra in the training set---is a non-trivial test of the physics-informed regularization: the radiative transfer constraint forces the inversion toward the parameter combination that is most consistent with the observed spectrum under the governing equations, even when that combination lies in the tail of the training prior.

The C$_2$/CN depletion reported by \citet{opitom20} is also recovered ($\log(Q(\text{C}_2)/Q(\text{CN})) \approx -0.6$), confirming that the framework distinguishes chemically distinct objects.

\subsubsection{Limitations of the Validation}

We caution that this validation is based on a single object ($N=1$ for validation, out of $N=2$ known interstellar comets). The 12\% MAD establishes feasibility---the framework produces outputs that are not grossly inconsistent with independent measurements---but does not constitute a statistical characterization of generalization error. The domain adaptation error $\delta_{\rm domain} \sim 5$--7\% estimated in \citet{[Paper~I]} is provisional and its variance across the (currently non-existent) interstellar comet population is unknown.
...
Table~\ref{tab:dust_results} reports the PINN-derived dust parameters for each synthetic input set. The reported uncertainties include the statistical error ($\sigma_{\rm stat}$), systematic error from model assumptions ($\sigma_{\rm sys}$, dominated by the spherical grain approximation), and domain adaptation error ($\delta_{\rm domain}$), combined in quadrature as described in \citet{[Paper~I]}.

\begin{deluxetable*}{lcccc}
\tablecaption{Framework Dust Parameter Outputs for Synthetic 3I/ATLAS Input Sets\label{tab:dust_results}}
\tablecolumns{5}
\tablewidth{0pt}
\tablehead{
\colhead{Parameter} & \colhead{Set A (Low)} & \colhead{Set B (Nominal)} & \colhead{Set C (High)} & \colhead{Uncertainty Budget}
}
\startdata
...
\enddata
\tablecomments{All values are PINN outputs on synthetic DIRTY inputs, not measurements of 3I/ATLAS. The uncertainty budget column gives the fractional uncertainty from the quadrature sum of $\sigma_{\rm stat}$, $\sigma_{\rm sys}$, and $\delta_{\rm domain}$. The heliocentric slope $n$ is from the $Q_d \propto r_h^{-n}$ fit.}
\end{deluxetable*}
...
\begin{deluxetable*}{lcccc}
\tablecaption{Framework Gas Parameter Outputs for Synthetic 3I/ATLAS Input Sets at $r_h = 3.0$~au\label{tab:gas_results}}
\tablecolumns{5}
\tablewidth{0pt}
\tablehead{
\colhead{Species / Ratio} & \colhead{Set A (Low)} & \colhead{Set B (Nominal)} & \colhead{Set C (High)} & \colhead{Uncertainty Budget}
}
\startdata
...
\textbf{Falsifiable prediction 4 (isotopes):} If future high-resolution spectroscopy ($R > 20000$) measures $^{15}$N/$^{14}$N and $^{13}$C/$^{12}$C in 3I/ATLAS, the framework predicts---based on the composition-disk chemistry scaling relations of \citet{oberg11}---that a CO/H$_2$O ratio of 0.3--0.6 corresponds to formation at $T \sim 25$--40~K, which would produce moderate isotopic fractionation ($^{15}$N/$^{14}$N elevated by a factor of 1.5--3 relative to solar). This prediction is indirect (it relies on the volatile-trapping model linking CO/H$_2$O to formation temperature and isotopic fractionation) and carries substantial uncertainty from both the formation model and the framework's parameter recovery error.

\subsection{Domain Adaptation Diagnostics}\label{sec:domain}

\subsubsection{Feature Alignment}

After adversarial domain adaptation, the domain classifier accuracy between solar system and synthetic interstellar comet features drops from 89\% to 52\% (chance = 50\%), and the MMD decreases from 0.47 to 0.12. These metrics indicate that the feature extractor learns representations that are statistically indistinguishable between the two domains. However, as discussed in \citet{[Paper~I]}, MMD reduction is a necessary but not sufficient condition for accurate physical parameter transfer: aligned features reduce domain-induced bias, but model misspecification error (spherical grains, Haser outflow) affects both domains equally and is not captured by domain-invariance metrics.
...
The known population of interstellar comets with published physical characterization consists of exactly two objects: 2I/Borisov and 3I/ATLAS. 2I/Borisov has real, published observations suitable for validation. 3I/ATLAS has only discovery astrometry and an approximate magnitude. This asymmetry defines what the framework can and cannot currently contribute:
...
\begin{deluxetable*}{lcccc}
\tablecaption{Interstellar Comet Properties: 2I/Borisov (Real) vs.\ 3I/ATLAS (Synthetic Case Study)\label{tab:comparison}}
\tablecolumns{5}
\tablewidth{0pt}
\tablehead{
\colhead{Property} & \colhead{2I/Borisov (Measured)} & \colhead{3I/ATLAS (Synthetic)} & \colhead{Solar System Median} & \colhead{Ref.}
}
\startdata
CO/H$_2$O & $>1.73$ & $0.46 \pm 0.09$ & $\sim$0.04 & 1, 2, 3 \\
$\log$(C$_2$/CN) & $-0.65 \pm 0.10$ & $-0.32 \pm 0.07$ & $\sim -0.25$ & 4, 3 \\
$Q_d$ at 2.1~au (kg~s$^{-1}$) & $\sim$35--45 & $43 \pm 4$ & --- & 5 \\
Polarization gradient (\%/1000~\AA) & $0.35 \pm 0.05$ & $0.12 \pm 0.03$ & $\sim$0.05--0.15 & 6 \\
$n$ ($Q_d \propto r_h^{-n}$) & $\sim$3.0 & $3.8 \pm 0.3$ & $\sim$3--6 & 5 \\
\enddata
\tablecomments{References: (1) \citet{bodewits20}; (2) This work (Set B synthetic input); (3) \citet{ahearn95}; (4) \citet{opitom20}; (5) \citet{clements21}; (6) \citet{bagnulo22}. ``---'' indicates insufficient homogeneous sample for a meaningful solar system median. \textbf{3I/ATLAS column values are synthetic inputs or framework outputs on synthetic inputs, not measurements.}}
\end{deluxetable*}
...
\begin{deluxetable*}{lccc}
\tablecaption{Observations Required to Test 3I/ATLAS Predictions\label{tab:required_obs}}
\tablecolumns{4}
\tablewidth{0pt}
\tablehead{
\colhead{Prediction} & \colhead{Required Observation} & \colhead{Instrument} & \colhead{Falsification Criterion}
}
\startdata
...
\enddata
\tablecomments{All predictions are from the Set B (Nominal) synthetic case study. Falsification of any prediction would not invalidate the framework---it would constrain the physical parameters of the real 3I/ATLAS to lie outside the Set B parameter range, providing scientifically valuable information.}
\end{deluxetable*}

\subsection{Limitations}

The limitations discussed in \citet{[Paper~I]} apply in full to this application study. We highlight those most relevant to the interpretation of the results above:
...
LSST is expected to increase the interstellar object detection rate, though estimates of the absolute rate remain uncertain \citep{jones18}. If the detection rate reaches $\sim$1 per year, a sample of 5--10 characterized interstellar comets could be accumulated within a decade. At that point: (1) $\delta_{\rm domain}$ can be empirically characterized across a range of compositions; (2) statistical comparisons between interstellar and solar system comet populations become possible; (3) the framework can be fine-tuned on real interstellar comet data rather than relying entirely on synthetic training and domain adaptation.

\section{Conclusions}\label{sec:conclusions}

We have applied the simulation-based deep learning framework of \citet{[Paper~I]} to two interstellar comets: 2I/Borisov (real archival observations, for validation) and synthetic test cases designed to approximate 3I/ATLAS (for hypothesis generation). Key findings:
...
We thank the observers and data analysts who produced the published 2I/Borisov datasets used for validation. Computational resources were provided by institutional HPC facilities. The code, trained model weights, synthetic input configurations, and DIRTY parameter files are available at \url{https://github.com/[repository]} and archived at \url{https://zenodo.org/record/[DOI]}.
\end{acknowledgments}

\section*{Data Availability}
The 2I/Borisov archival data used for validation are available through the ESO Science Archive Facility (\url{http://archive.eso.org}) and the Mikulski Archive for Space Telescopes (\url{https://archive.stsci.edu}). The synthetic 3I/ATLAS input datasets (Sets A, B, C) generated for this study, together with the DIRTY configuration files used to produce them, are available at \url{https://github.com/[repository]} and archived at \url{https://zenodo.org/record/[DOI]}. The framework code and trained model weights are available at the same repository. The 3I/ATLAS discovery MPEC is available through the Minor Planet Center (\url{https://minorplanetcenter.net}).
