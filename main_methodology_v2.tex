...
\begin{abstract}
Interstellar comets challenge traditional cometary analysis: their intrinsic faintness (SNR~$\sim$1--5 at discovery), structured background noise from dense star fields, and systematic composition differences relative to solar system comets push conventional methods into regimes where they produce unreliable results. We present \textsc{Cometh} (COmet MEasurement via deep learning and Hard-constraints), a multi-modal deep learning framework combining: (1) multi-scale CNNs with squeeze-and-excitation attention and Grad-CAM diagnostics for faint feature detection; (2) physics-informed neural networks (PINNs) with hard physical constraints---mass and momentum conservation embedded in the network architecture---for dust and gas parameter inversion; and (3) a meta-learning framework with adversarial domain adaptation using physically motivated, DIRTY-simulated task distributions for few-shot transfer from solar system to interstellar comets. Three auxiliary innovations---attention consistency loss, an out-of-distribution detector, and an adversarial observing strategy optimizer---further improve robustness. We provide complete architectural specifications sufficient for independent reproduction. Under a deliberately pathological stress test suite, \textsc{Cometh} achieves a 2.8$\times$ SNR gain over traditional methods and $\sim$8\% error on dust production rate estimation, while traditional methods produce scientifically misleading results. A controlled human vs.\ machine blind test confirms that human experts are also unreliable at interstellar comet SNR levels (true-positive rate 31\% vs.\ 78\% at SNR~$<2$). A proxy validation on comet C/1995 O1 (Hale-Bopp) partially bridges the solar system--interstellar domain gap. We emphasize that this is a benchmarking and methodology paper: all metrics are evaluated against known synthetic ground truth, and the single-object consistency check on 2I/Borisov ($N=1$) is a feasibility demonstration. The framework's failure boundaries are documented as carefully as its successes. Code, trained weights, and synthetic datasets are available as supplementary material through the AAS manuscript submission system.
\end{abstract}
