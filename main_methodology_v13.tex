...
\begin{abstract}
Interstellar comets 2I/Borisov and 3I/ATLAS are observed at SNR~$\sim$1--5, below the validation regime of traditional single-modality cometary analysis. We present \textsc{Cometh}, a physics-constrained multimodal fusion framework integrating: (i) a multi-scale CNN with squeeze-and-excitation attention, achieving 2.8$\times$ detection-image-quality gain (PSNR-based) over traditional baselines at SNR~$\sim$2; (ii) a physics-informed neural network (PINN) with hard conservation constraints embedded via architectural reparameterization, yielding $\sim$8\% error on dust production rate on synthetic benchmarks ($\sim$12\% MAD vs.\ published 2I/Borisov values); and (iii) cross-modal attention with a self-diagnosing out-of-distribution detector preventing silent extrapolation failures. Domain adaptation via physically motivated meta-learning reduces synthetic-to-synthetic domain shift; the synthetic-to-real gap remains unclosed. A same-data comparison on 231 HST/WFC3 and VLT/FORS2 images of 2I/Borisov yields 201 Af$\rho$(1000\,km) measurements across 11 epochs; HST and FORS2 agree within $\sim$3\% (ratio 1.03), validating the traditional-method baseline for solar-system-like compositions. A stress-test suite maps failure boundaries; the spherical-grain assumption ($\sim$8\% bias on $Q_d$) is the primary systematic limitation. A Disaster Preview shows traditional pipelines would misinterpret extreme-composition comets (CO/H$_2$O underestimated by up to 50$\times$), and a pilot study ($N=5$) suggests human experts miss $>2/3$ of faint features at SNR~$<2$. No \textsc{Cometh} inference was performed on real 3I/ATLAS data; all ``3I/ATLAS'' results are synthetic case studies. The framework is proposed with preliminary validation ($N=1$ real object, $\sim$12\% MAD), not as a fully validated measurement tool. Code at \url{https://anonymous.4open.science/r/cometh-F185}; trained weights at Zenodo (DOI embargoed until acceptance).
\end{abstract}
